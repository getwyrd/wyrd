...
> **Supersedes [proposal 0007](../accepted/0007-milestone-4-production-metadata-backend.md)**
> (2026-07-02), the original M4 plan, which is retained unchanged as a frozen record. This
> document carries the **current** M4 plan; where the two differ, this one governs. The changes are
> **review-pass refinements** — a deployment-scope correction plus risk/anchor hygiene — not a
> re-plan of the metadata work (the TiKV backend, its transactional mapping, and the DST discipline
> carry over unchanged). See "Changes from proposal 0007" below.
...
## Changes from proposal 0007

This revision supersedes [0007](../accepted/0007-milestone-4-production-metadata-backend.md) with
review-pass refinements; the metadata swap, the TiKV transactional mapping, and the DST discipline
carry over unchanged. What differs:

- **Deployment scope (the material change).** 0007's "peers discovered through L5" deployment
  definition-of-done silently depended on components M4 excludes and that do not exist — a
  cross-process **etcd-backed `Coordination`** backend, plus **runnable gateway/custodian process
  roles** (etcd discovery was deferred in M2, [0004][p4]; the custodian binary was deferred in M3,
  [0005][p5]; only the process-local `coordination-mem` exists). This revision adds a **Deployment
  prerequisite** note naming that as an explicit prerequisite (etcd is Coordination's ADR-0006
  *second implementation*, exactly as TiKV is `MetadataStore`'s), reconciles the "Coordination is
  untouched" line (it means the *metadata swap* touches neither the Coordination trait nor its
  consumers — not that a distributed backend exists), and **gates slice 5 and the deploy graduation
  criterion** on it (static endpoints until the prerequisite lands).
- **`tikv-client` maturity risk named.** The production-durability tier rests on the pre-1.0 (0.4.x)
  Rust client; this revision states the maintenance/supply-chain risk and adds an evaluation gate +
  fallback stance before the dependency is committed.
- **Large-directory `scan` guard.** Promoted from open question to a known limitation with an
  interim paging cap — a sharper cliff on the network backend than on redb.
- **Tier-2 vs slice-5 clarified.** Tier-2 (single machine, I/O honesty) is a distinct run from
  slice 5's multi-node topology, not one box standing in for the production shape.
- **Citation hygiene.** Three drifted line-number anchors replaced with symbol references.
