...
Both are required, and they are required in the same release as the S3 verbs that create
...
**Structural validity is checked at decode, never defaulted ([ADR-0045][a45]).** Every record
...
```text
                       CreateMultipartUpload
                                │  reserve: read mpuctl:count < MAX_SESSIONS,
...
|---|---|---|---|
| Create session | **counter present:** `require(mpuctl:count == c && c < MAX_SESSIONS)`; **counter absent (fresh/upgraded store, reads as 0):** `require_absent(mpuctl:count)`; plus `require_absent(mpu:<id>)`, bucket-existence per [ADR-0046][a46] §4 | 1 put session, and either 1 CAS `mpuctl:count` `c→c+1` **or** (bootstrap) 1 put `mpuctl:count = 1` | O(1), < 1 KB |
...
| Part intent | `require(mpu == Open@E)` (the finding-1 serialization edge — a read precondition, not a write, so concurrent intents never conflict) | one owned `sidx:<id>:<part>:<chunk>` staging put per chunk, each its own commit | O(1) per commit |
...
| Root flip (publish) | `require(mpu == Completing@E)`; the bucket-existence pair of [ADR-0046][a46] §4; `require_absent(inode)`+`require_absent(dirent)` **or** `require(inode == prior)` | 1 put session→`Completed`, 1 put inode root (≤ `V`, records the segment-group `<id>:<E>` if segmented), ≤ 1 put dirent, 1 put `retire:records:{parts}`, ≤ 1 put `retire:bytes:` for unnamed staged parts, ≤ 1 put `retire:bytes:{generation}` for the superseded generation | ≤ 4·V + O(1) |
...
| Abort / reap fence | `require(mpu == Open@E)` (or `Completing@E` for the rollback, and for the **restore fence** below) | 1 put, 1 put `retire:bytes:{session}`, ≤ 1 put `retire:records:{seg:<id>:<E>}` | O(1) |
...
| Retirement drain step | `require(retire:… == prior)` (single-winner on the obligation) **and** `require_absent(orphan:<pos>)` per mark written (decision 4.2) | `B` orphan puts **or** `B` record deletes, plus 1 cursor put | ≤ `E_tx/2`; `B = ⌊(E_tx/2)/(bytes per mutation)⌋` (~1,000 small orphan marks; fewer for larger record deletes) |
...
| Segment repoint (reconstruction / rebalance of a committed segmented object, decision 2/7) | `require(seg:<id>:<E>:<i> == prior)`, `require(inode == prior)`, **`require(orphan:<P_new> == prior)`** (the pre-mark must still stand — see below) | pre-step: 1 put `orphan:<P_new>` pre-mark under `require_absent(orphan:<P_new>)` (own batch, before the fragment write, decision 4.2); then the CAS batch: 1 CAS `seg:` (placement `P_old→P_new`) + 1 delete `orphan:<P_new>` (adopt) + 1 put `orphan:<P_old>` under `require_absent` (evidence the vacated source) | O(1); a lost CAS is a no-op that leaves the `P_new` pre-mark standing for GC — neither branch strands a fragment (X47, adversary C) |
| Staged part re-place (reconstruction of an in-flight owned chunk, decision 2/5) | `require(mpu == Open@E)`, `require(part:<id>:<n> == prior)`, **`require(orphan:<P_new> == prior)`** | pre-step: 1 put `orphan:<P_new>` pre-mark under `require_absent(orphan:<P_new>)` (own batch, before the fragment write, decision 4.2); then the CAS batch: 1 CAS `part:` (chunk placement `P_old→P_new`) + 1 delete `orphan:<P_new>` (adopt) + 1 put `orphan:<P_old>` under `require_absent` (evidence the vacated source) | O(1); a lost CAS (the session left `Open@E`, or the part moved) is a no-op that leaves the `P_new` pre-mark standing for GC — the rebuilt staged fragment is never stranded (X29, finding 1) |
| Owned-`sidx:` drain step (Aborting **and** Completed) | `require_absent(orphan:<pos>)` for **every mark this batch writes** — the grace-preserving guard (decision 4.2, iteration-7 finding 3); the walk itself is reference-based (decision 5), so there is no obligation record to serialize two drainers on | `B` orphan puts + owned `sidx:` deletes, 1 cursor put | ≤ `E_tx/2` (byte-budgeted, ~1,000 small marks); a guard loss re-reads and re-splits, marking strictly fewer positions — it never re-stamps a live grace clock |
...
`orphan:` mark carries `require_absent(orphan:<pos>)` (decision 4.2); both are omitted from the
...
offer. Building the staged set costs a **product of chunks**, not
...
or in-flight — lands on a draining server**. The staged population on a draining server is
...
2. **Evidence precedes the lifting of protection, always.** The drain writes `orphan:` records
   ...
   **An `orphan:` mark is written only when absent — never re-stamped (normative; iteration-7
   ...
   the owned-`sidx:` walk, the repoint pre-marks — therefore carries `require_absent(orphan:<pos>)`,
   and a writer that finds the mark already present treats the fragment as **already evidenced**:
...
- **An unknown reserve outcome is settled by re-reading, never by re-reserving.** This is why the
  ...
  **its own** `attempt_id` means the reserve landed and it *adopts* that slot; any other value, or
  absence, means it did not, and it probes the next free index. Blindly probing a fresh index after
  an unknown outcome would leak the landed one for the session's whole life, and a session whose
  slots leak one per unknown result wedges at `MAX_INFLIGHT_PARTS` with no crash to blame — the
...
part start, one keyed delete per part end, and the per-chunk `require(mpu == Open@E)` read
precondition on each `sidx:` intent — a read, which does not serialize concurrent writers. That is
...
- **The liveness observable, derived only from records the protocol durably writes.** A session's
  *last progress instant* is
  ...
  max over its slot records of reserved_at_millis)`, read via the bounded `psum:<id>:` and
  `slot:<id>:` ranges — **summaries, not the fat part records** (§1) — and a session additionally counts as live while **any session-owned `sidx:`
...
- **Admission control — the enforceable, exact bound (**D-C**, F7/F12).** `CreateMultipartUpload`
  ...
  `mpuctl:count` — **an absent record on a fresh or upgraded store reads as 0**, and the first
  Create initializes it in the same batch with `require_absent(mpuctl:count)` + `put = 1` rather
...
one reaper pass (dispatched from the fenced control point, like every custodian loop):
  ...
  2. for each session, in id order:               # deterministic order, resumable
       if clock_source not ours:      alarm; skip  # FIRST — guard from the session record (F10/6b)
                                                  #   its exit is the operator verb, not this pass
       progress = max(created_at,                          # scan("psum:<id>:") — <= MAX_PARTS
                      max part committed_at,               # scan("slot:<id>:") — <= MAX_INFLIGHT
                      max slot reserved_at)                #   pre-first-chunk parts (finding 2)
       if state == Open and ((now - progress > W_open and no live sidx:/slot: lease)
                             or (now - created_at > W_session)):
             CAS Open@E -> Aborting@E+1 + put retire:bytes:{session, all}  # one O(1) batch
       if state == Completing and now - fenced_at > W_completing:
       ...
       terminal = (state == Completed and now - completed_at > W_tombstone)
               or (state == Aborting)
...
| X53 (finding 2) | The **first** `CreateMultipartUpload` on a **fresh or upgraded** store, where `mpuctl:count` does not yet exist | The create reads `mpuctl:count` **absent-as-0** and initializes it in the same batch with `require_absent(mpuctl:count)` + `put = 1` (rather than the CAS the steady-state path uses), so the first session is admissible with **no migration/init step**; a concurrent first-create that initialized it first makes the loser's `require_absent` fail and it retries against the re-read `1`. Without this the Create CAS `require(mpuctl:count == c)` can never be satisfied and multipart is dead on a new store | D6 (**D-C**, F12, finding 2), §1 |
...
- `crates/traits/src/lib.rs` — **the one narrow-seam change this protocol requires** (ADR-0010 /
  ...
  `SCAN_CAP`. Today's `scan(prefix)` is prefix-only and complete-or-`ScanCapExceeded`
...
- `crates/core/src/write.rs` — staged placement (committed **and** in-flight owned) selects against
  ...
  `require(mpu == Open@E)`** (finding 1); `UploadPart` claims its `slot:<id>:<k>` index before the
...
| **FU-6** | *Operator-driven session abort* — a management verb that fences any session to `Aborting` on the operator's authority, plus the alarm that names a skipped foreign-clocked session; the exit for a session the reaper declines to judge (X26) | management surface (proposal 0008) | with #625 (the alarm) / on the first foreign-clock report (the verb) |
...
- **Tier-0 DST ([ADR-0009][a9]) is the correctness authority** for every interleaving named in
  ...
  6's finding-2 row), and two concurrent drainers over one owned `sidx:` range (X56)**, and
  arrival-outruns-drain. Each lands as a seeded regression.
...
serialize. What remains is one per-chunk `require(mpu == Open@E)` **read** precondition on each
`sidx:` intent — a read does not serialize concurrent writers — and the cap stays *enforced*, not
