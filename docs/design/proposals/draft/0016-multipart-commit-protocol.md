...
| `Completing` | `Completed` (root flip won), `Open` (publish lost, request rejected as invalid, or reaper rollback after `W_completing` from `fenced_at`), `Aborting` (**three doors, all installing `retire:records:{seg:<id>:<E>}` in the fencing batch**: the **restore fence** (**D-B**), the **`W_session` residency ceiling** measured from `created_at` (iteration-10 finding 7), and the **operator abort** (FU-6, iteration-10 finding 8) — the transitions that leave `Completing` without passing through `Open`, decision 3) | client; reaper; the restore pass; the operator |
...
| **`Completing` → `Aborting` fence** — restore (**D-B**), `W_session` ceiling (iteration-10 finding 7), or operator abort (FU-6, finding 8): **one shape for all three doors** | `require(mpu == Completing@E)` | 1 put session→`Aborting@E+1`, 1 put `retire:bytes:{session, parts}`, **1 put `retire:records:{seg:<id>:<E>}`** for the segments that attempt already wrote | O(1) — one batch, so no interleaving leaves a `Completing` session whose segments have no reclaimer |
...
- **Abandonment rule (two arms).** A session in `Open` is fenced to `Aborting` when **either**:
  ...
  in one batch that also installs `retire:records:{seg:<id>:<E>}` for the segments that attempt
       ...
       if not foreign and state == Completing:
             ...
             if now - created_at > W_session:
                   CAS Completing@E -> Aborting@E+1 + put retire:bytes:{session, parts}
                                                     + put retire:records:{seg:<id>:E}
             elif now - fenced_at > W_completing:
                   CAS Completing@E -> Open@E+1 + put retire:records:{seg:<id>:E}  # rollback;
                                                                                   #  only THIS epoch
