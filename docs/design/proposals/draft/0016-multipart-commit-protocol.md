...
|---|---|---|---|---|
| `mpuctl` | **one record, three fields: `{ count, max_sessions, profile }`** (the admission record, decision 6). `count` is the number of `mpu:` records that exist in any state; `max_sessions` is the **governing limit those increments were admitted against**, and `profile` is the budget tuple that *establishes* it (`W_ref`, `MAX_PART_CHUNKS`, `MAX_PARTS_PER_SESSION`, `MAX_INFLIGHT_PARTS`, `MAX_STAGED_CHUNKS`) — stored because equal quotients can hide unequal footprints (iteration-10 finding 4), stored rather than derived per gateway so a rolling configuration change cannot leave two gateways enforcing different bounds (iteration-9 finding 5, X64). Both fields live in **one** record and every mutation CASes it **whole** (`require(mpuctl == prior)`), so the count and the limit it was checked against can never be read apart. A gateway whose local **profile** differs from `prior.profile` **refuses to admit and alarms**; changing it is an explicit operator CAS gated on the live population having drained | **absent reads as `{ count: 0 }`** — the **first** Create on a fresh or upgraded store initializes it in the create batch with `require_absent(mpuctl)` + `put mpuctl = { count: 1, max_sessions, profile }` (no migration/init step), so a fresh store adopts the first admitting gateway's derivation and every later gateway is checked against it; every later Create CASes the whole record with `count + 1`; CAS'd `-1` in the **terminal** batch that deletes the last session record — that batch also `require`s the session record's **exact prior bytes** (`require(mpu:<id> == prior)`), so the `-1` is **exactly-once** even when a gateway drain and the reaper both attempt the teardown (one wins the CAS, the other's precondition fails and its batch is a no-op) | never deleted (a fixed singleton) | Create (read-then-CAS reservation); **and the custodian/reaper, which compares `profile` against its own configuration and fails closed before building the reference set** (iteration-13 finding 7) |
...
| `retire:records:<token>` | `{session, parts}` and/or `{seg: <group-nonce>:<epoch>}` — staging records (the **published** parts' records **with their `psum:` summaries** — never the parts Complete left unnamed, whose records ride their own `retire:bytes:{parts}` obligation so that marking always precedes deletion, iteration-14 finding 3 and/or the dangling segments of **one** rolled-back Completing attempt) to delete whose bytes are protected by something else | the publication batch (part records), or a Completing rollback/abort (that attempt's dangling segments) | the retirement drain | the reaper, via cursor-keyed **bounded key ranges** |
...
**Segment groups must NOT be keyed by the upload id, because they outlive it (iteration-14
finding 2).** The claim above is exactly true for the *session* record and false for the segments a
session publishes: a `Completed` session's `mpu:` record is deleted when its tombstone window
elapses, while its `seg:<g>:<epoch>:*` records **must survive for as long as the object they map
does**, which may be years. So the one guard the design offers — `require_absent(mpu:<id>)` — is
already gone by the time it matters: a colliding id would find the session key free, start its
epoch sequence from the same values, and its segment-write phase would **overwrite a live object's
segment records**, corrupting that object's map rather than returning the `Conflict` claimed here.
...
```text
   ...
   │                             ├─ W_session ceiling ────► Aborting@E+2 ──► (drain) ─► ∅ (count-1)
   │                             └─ operator abort (FU-6) ► Aborting@E+2 ──► (drain) ─► ∅ (count-1)
...
deterministic segment-group nonce, decision 7). The
...
| `Completing` | `Completed` (root flip won), `Open` (publish lost, request rejected as invalid, or reaper rollback after `W_completing` from `fenced_at`), `Aborting` (**three doors, all installing `retire:records:{seg:<g>:<E>}` in the fencing batch**: the **restore fence** (**D-B**), the **`W_session` residency ceiling** measured from `created_at` (iteration-10 finding 7), and the **operator abort** (FU-6, iteration-10 finding 8) — the transitions that leave `Completing` without passing through `Open`, decision 3) | client; reaper; the restore pass; the operator |
...
**Batch size is bounded by BYTES *and* by OPERATIONS — the envelope is 10 MB **and** 5 s (iteration-13 finding 3).** Every drain and
...
**Bytes alone do not prove termination.** The inherited envelope is `E_tx = 10 MB / 5 s`, and the
byte budget only addresses the first half. Every backend applies a batch's preconditions and
mutations **sequentially inside the transaction**: TiKV awaits each `get_for_update` / `put` /
`delete` in turn (`crates/metadata-tikv/src/lib.rs:1063-1095`), and FDB likewise awaits every
precondition read (`crates/metadata-fdb/src/lib.rs:1425-1438`). So a batch of ~1,000 small
`orphan:` marks — comfortably under 5 MB, and exactly the `MARK_BATCH` precedent this document
cites — can exceed the **5-second** deadline and time out indefinitely, and the reaper's idle fence
is worse: under the `SCAN_CAP` clamp alone it can pin tens of thousands of slot preconditions in one
batch. A batch that always times out is not slow, it is **stuck**, and the affected work (a drain
step, a fence, a terminal delete) then never completes at all.

`B` is therefore `min(B_bytes, B_ops)`: the byte-derived count above, **and** an
operation-derived cap `B_ops` calibrated so that a batch's sequential round trips fit the
transaction deadline with margin on the slowest supported backend. `B_ops` is a knob with the same
status as `B_bytes` — deployment-tunable inside a range, never a magic number — and every row of
the batch inventory whose mutation or precondition **count** can grow (the drain steps, the owned
`sidx:` walk, the idle fence's slot pins, the terminal delete's slot deletes) is bounded by both.
For the batches that cannot be split — the terminal delete and the fence's pin — `B_ops` is what
makes the *knob* legal rather than the batch, which is why `MAX_INFLIGHT_PARTS` now carries it as a
clamp (finding 2's row).
...
| Segment write (one, decision 7) | `require(mpu == Completing@E)` | `B_seg` `seg:<g>:<E>:` puts (each ≤ `V`), 1 cursor put | ≤ `E_tx/2 = 5 MB`; `B_seg = ⌊(E_tx/2)/V⌋ = 50` seg puts (byte-budgeted, **not** a fixed count) |
| Root flip (publish) | `require(mpu == Completing@E)`; the bucket-existence pair of [ADR-0046][a46] §4; **the target dirent is pinned on BOTH branches** — `require_absent(dirent)`+`require_absent(inode)` for a fresh key, **`require(dirent == prior)` *and* `require(inode == prior)`** for an overwrite (iteration-9 finding 6: publication is defined against the *dirent identity*, so guarding only the inode lets a `metadata::rename` move that dirent after Complete resolved it and the flip then overwrites an inode now bound at another name, leaving the multipart target absent or rebound while the client is told the Complete succeeded). Either binding changing is a `Conflict` that re-resolves and retries within `R_publish` | 1 put session→`Completed`, 1 put inode root (≤ `V`, records the segment-group `<group-nonce>:<E>` if segmented), ≤ 1 put dirent, 1 put `retire:records:{parts}` **naming only the PUBLISHED parts** (iteration-14 finding 3), ≤ 1 put `retire:bytes:{parts}` for the unnamed staged parts — **which names their bytes *and* their records**, so the same obligation marks then deletes them, ≤ 1 put `retire:bytes:{generation}` for the superseded generation | ≤ 4·V + O(1) |
| Fence release (publish lost / invalid) | `require(mpu == Completing@E)` | 1 put session→`Open`, ≤ 1 put `retire:records:{seg:<g>:<E>}` for this attempt's segments | O(1) |
| Abort / reap fence | `require(mpu == Open@E)` (or `Completing@E` for the rollback, and for the **restore fence** below). **The reaper's *idle* arm additionally pins the liveness evidence it judged: `require(slot:<id>:<k> == prior)` for every slot index observed present and `require_absent(slot:<id>:<k>)` for every one observed free** (≤ `MAX_INFLIGHT_PARTS`, iteration-9 finding 4) — a renewal, release or new reservation in the read→fence window makes the batch `Conflict` and the pass re-derives. A **client** Abort and the `W_session` arm carry no slot preconditions | 1 put, 1 put `retire:bytes:{session}`, ≤ 1 put `retire:records:{seg:<g>:<E>}` | O(1) |
| **`Completing` → `Aborting` fence** — restore (**D-B**), `W_session` ceiling (iteration-10 finding 7), or operator abort (FU-6, finding 8): **one shape for all three doors** | `require(mpu == Completing@E)` | 1 put session→`Aborting@E+1`, 1 put `retire:bytes:{session, parts}`, **1 put `retire:records:{seg:<g>:<E>}`** for the segments that attempt already wrote | O(1) — one batch, so no interleaving leaves a `Completing` session whose segments have no reclaimer |
| **GC reclaim-intent step** (iteration-10 finding 1, GC-side ordering) | `require(orphan:<pos> == prior)` — the exact bytes GC read | 1 CAS `orphan:<pos>` → `reclaiming`, committed **before** `delete_fragment`; the key delete follows after, as today | O(1) per position, batched with the sweep's other marks inside `E_tx/2`; this is what lets an adoption CAS preconditioned on the pre-mark's original bytes fail from the instant reclamation starts, with no clock on either side |
...
| Fence a resurrected **`Completing`** session as if it were `Open` (iteration-7 adversary, X57) | Snapshot a session in `Completing@E` **after** its segment writes and **before** its root flip, restore, and run the fence pass: the session MUST take the `Completing → Aborting` restore transition, whose batch installs `retire:records:{seg:<g>:<E>}`, and that range MUST drain empty. An abort fence that only installs `retire:bytes:{session}` leaves those `seg:` records with no deleter in the whole design — no root flip means no inode ever adopts or supersedes them, and no rollback means no obligation names them. |
...
- **A tombstone answers a retry, not any request (iteration-10 finding 9).** Within the
  `W_tombstone` window a `Completed` session answers `200` with the recorded ETag, which is what
  makes Complete idempotent across a client retry. It must be idempotence keyed on **request
  identity**, not on the upload id alone: a client that reuses the id with a *different* part list
  would otherwise be told **its** assembly succeeded while the stored object holds the earlier one —
  a silent wrong-answer, worse than any error. The `Completed` record therefore carries
  `complete_fingerprint`, a digest over the ordered `(part_number, digest)` pairs the winning
  Complete named, and the tombstone returns `200` + the recorded ETag **only** when the incoming
  request's fingerprint matches. A mismatch is `404 NoSuchUpload` — the honest answer, since the
  upload id is consumed and no session exists that could publish the assembly being asked for, and
  it is exactly what the same request would receive one tick after the window closes.
- **The two publication obligations must not be able to race each other (normative; iteration-14
  finding 3).** The flip installs both `retire:records:{parts}` — records whose bytes are now
  protected by the published inode — and, when Complete named only a subset, `retire:bytes:` for
  the **unnamed** staged parts. Those two are drained independently, by a gateway inline or by the
  reaper, in either order. If `retire:records:` named *all* the parts, it could delete the unnamed
  parts' `part:` records **first** — and those records are both the only thing protecting those
  chunks and the only source of their placements, so the byte obligation would then have nothing to
  orphan-mark from: durable bytes left unreferenced **and** unevidenced, which is outcome (a)
  installed by the publication path itself. The fix is a scoping rule, not an ordering one, because
  an ordering rule between two independently drained obligations is exactly what this design avoids
  everywhere else: **`retire:records:{parts}` names only the PUBLISHED parts**, and the unnamed
  parts go wholly into `retire:bytes:{parts}`, which already has the right shape — §1's token
  grammar defines a `retire:bytes:` payload as *"bytes to orphan-mark, **then** the naming records
  to delete"*, one obligation that marks before it deletes, with no cross-obligation dependency to
  get wrong. Every part record is then named by exactly one obligation, and each obligation's own
  internal order is what it already was.
...
- **Retry after an unknown outcome (F5).** The root flip changes the session record to
  ...
  `seg:<g>:<E>:<i>` key and frozen content, decision 7d) and retries the flip; `Open@E+1` →
...
| `CompleteMultipartUpload` | fences, publishes | `409 OperationAborted` | `404 NoSuchUpload` | `200` + recorded ETag **only for an identical retry** (matching `complete_fingerprint`); a different body ⇒ `404 NoSuchUpload` (iteration-10 finding 9) | `404 NoSuchUpload` |
...
2. **Evidence precedes the lifting of protection, always.** The drain writes `orphan:` records
   ...
   **An `orphan:` mark is never re-stamped by a DUPLICATE of the retirement that wrote it, and is
   always replaced by a LATER unreference event — the three-arm rule below (normative; iteration-7
   finding 3 as corrected by iteration-9 finding 3 and iteration-13 finding 1).** The blanket
   "written only when absent, never re-stamped" this sentence used to state is the concurrency half
   alone; asserted on its own it re-opens the reader-safety hole, because an already-expired mark
   from an earlier event would then survive the removal of the last reference and let GC reclaim
   with no grace at all. `orphaned_at_millis` **is** the grace clock GC evaluates
   ...
   **The value encoding, and the legacy marks already on disk (iteration-10 finding 3).** Today an
   ...
   (`gc.rs:315-330`) — so adding an event identity is a **value-schema change with legacy data in
   the field**, and it needs the dual-format contract [ADR-0045][a45] requires rather than a silent
   reinterpretation. The value becomes `{ orphaned_at_millis, event }` under the project's ordinary
   additive-field encoding — **three variants in total** once GC's reclaim-intent state is counted
   (legacy bare decimal, structured, and structured-with-`reclaiming`, below) — and:
   - **decoding is dual-format**: a value that parses as a bare decimal is a **legacy mark**,
     carrying `orphaned_at_millis` and **no** event identity; a structured value carries both. Both
     forms are readable by an upgraded custodian, and a legacy mark's timestamp keeps its meaning
     exactly (the grace window is unchanged for it);
   - **a legacy mark has, by definition, an unknown identity, so it takes the "different identity"
     arm** — the first unreference event that meets it replaces it with a structured value and a
     fresh stamp. That is the safe direction (a fresh reader-grace window rather than reclamation
     against an ancient stamp), and it is self-limiting: the replacement writes an identity, so the
     second drainer of that same event reads its *own* identity and skips. Legacy marks therefore
     convert on contact, with no migration pass;
   - **encoding is not byte-identical across the change**, so the round-trip rule applies in the
     one direction that matters: a legacy value decodes and re-encodes as the *same legacy value*
     until an event replaces it, so an upgraded custodian that merely reads the ledger never
     rewrites it and never restarts a grace clock (the `already_marked` property);
   - a **downgraded** custodian reading a structured value fails its bare-`u64` parse and today
     silently drops that entry from `orphan_leases` (`gc.rs:315-330`), which makes the fragment
     *unreclaimable* rather than wrongly reclaimed — the safe direction, and the same
     custodian-before-gateway ordering the Backward compatibility section already mandates covers
     it.

   With that in place the three arms read:
   ...
   **An installed-but-undrained `retire:` obligation PROTECTS the bytes it names (normative;
   iteration-12 finding 5).** The re-stamp above happens in the drain, which is asynchronous — so
   between the batch that removes the last reference and the drain that refreshes a stale mark
   there is a window in which GC sees an unreferenced fragment carrying an **already-expired**
   mark, transitions it to `reclaiming` and deletes it. A GET overlapping that deletion gets none
   of the grace the refresh exists to give it — the defect this rule was added to fix, merely moved
   into the gap. Serializing the refresh into the unreference batch is not available: that fan-out
   is exactly what `retire:` exists to keep out of that batch. So the **obligation protects — but as a
   KEYED LOOKUP, never a materialized set (iteration-13 finding 4).** Saying "`protects()` returns
   true for every position named by an installed `retire:bytes:` obligation" would have required GC
   to *expand* every pending obligation in memory, and `retire:` is the one namespace this design
   explicitly declines to bound by cardinality — with a single segmented generation expanding to
   ~1.78 M positions, a reconcile pass's memory and work become unbounded and the pass OOMs or never
   finishes, disabling GC. Paginating the read bounds each read, not the aggregate. That would have
   traded a reader-safety window for a liveness failure of the whole collector.

   The bounded form uses the identity the mark already carries. Every mark names its unreference
   event, and an event's token **is** its obligation key, so GC's test on a candidate position is a
   single `get` of `retire:bytes:<token(mark.event)>`: **present ⇒ that retirement is still
   draining ⇒ protected; absent ⇒ drained ⇒ the mark's own grace governs.** O(1) memory per
   position, no expansion, and it composes with the page-wise walk of finding 4 in the previous
   round.

   That leaves the case the keyed lookup cannot see — a mark whose event is *older* than the
   retirement now in flight (the stale mark this rule is about), whose own obligation drained long
   ...
   the drain — the direction this design already accepts everywhere ("never earlier than today's
   inline orphaning, only later").
   ...
   **A caller-side deadline cannot fix this, and the iteration-9 attempt to do so repeated the very
   defect it was fixing (iteration-10 finding 1).** Requiring the adoption CAS to "refuse if its own
   pre-mark is older than `W_repoint`" is a check evaluated in the *caller*, before the commit —
   exactly the shape that made the renewal/reap race real (X62). The transaction can be delayed
   after passing the check, including through a `CommitUnknownResult`, until GC has crossed
   `G_orphan`, deleted the fragment and left the unchanged key pending cleanup; the late CAS then
   satisfies `require(orphan:<P_new> == prior)` and publishes a placement naming missing bytes. A
   pre-commit timestamp check bounds when a batch is *issued*, never when it *applies*, and only
   the latter matters here.

   **The sound fix is to make GC record its reclamation in metadata BEFORE it destroys the bytes,
   so the precondition can witness it (normative).** GC's current order is the protocol's own
   "intent precedes bytes" rule run backwards: it calls `delete_fragment` inside the per-server
   loop and only afterwards commits the matching key delete (`gc.rs:189-207`), so for the length of
   a sweep the ledger asserts a fragment that no longer exists. GC therefore **CASes
   `orphan:<pos>` into a terminal `reclaiming` state — an exact-value CAS from the bytes it read —
   and commits that batch *before* calling `delete_fragment`**; the key itself is deleted after, as
   today.

   **`reclaiming` is a third value variant with defined decoding and restart (iteration-11
   finding 5).** A crash *between* that CAS and the `delete_fragment` is not exotic — it is the
   ordinary crash this ordering deliberately opens a window for — and it leaves a durable
   `reclaiming` value with the fragment **still present**. So the variant belongs to the value
   schema, not to an implementation: the value is `{ orphaned_at_millis, event, reclaiming: true }`,
   `orphan_leases` **decodes it rather than rejecting or skipping it** (a rejection would abort the
   whole GC pass at its decode boundary; a silent skip would make the fragment permanently
   unreclaimable, since nothing else ever revisits the position), and it reads as **"reclamation
   already decided; resume it"**. The next pass re-issues `delete_fragment` for that position —
   idempotent, and a no-op if the earlier attempt landed — and then deletes the key, with **no**
   re-evaluation of the grace test: that window already elapsed once, and the `reclaiming` CAS did
   not disturb the stamp it was measured from. Restart is therefore idempotent and terminating, and
   a fragment is reclaimed at most once per mark. Then **any** adoption CAS still preconditioned on the pre-mark's original bytes fails
   from the instant reclamation begins, whatever its latency, whatever the commit path, and with no
   clock read on either side. Safety rests on the precondition again — but now on a precondition
   that can actually observe the thing it needs to, which is what the iteration-8 text wrongly
   assumed and the iteration-9 text tried to replace with timing.

   That closes the *adoption* end. **The `W_repoint` deadline still has real work to do at the
   other end — gating the destination WRITE (iteration-11 finding 7).** A worker that pauses after
   pre-marking but *before* authorizing the D-server write is not protected by the ordering above:
   GC ages the mark through `G_orphan`, CASes it `reclaiming`, deletes nothing (no fragment is
   there yet), removes the mark — and when the worker resumes, its newly authorized write lands on
   a position with **no reference and no evidence**, the adoption CAS having lost. That is outcome
   (a), a strand. `W_write` cannot cover it, because `W_write` bounds only the interval *after*
   authorization. So the worker **MUST NOT authorize the destination write if its own pre-mark is
   older than `W_repoint`** — it restarts from a fresh pre-mark instead — and the sizing is
   **`G_orphan > W_repoint + W_write + δ_clock`** (strict), so any write this rule permits lands
   strictly inside its evidence's grace.

   The distinction from the mistake this section already records is the point, and it is worth
   stating rather than leaving to inference: a caller-side deadline can soundly gate a **caller-side
   action** — whether this worker issues a network write — because the caller is the one performing
   it. What it cannot do is bound when a **transaction applies**, which is what the iteration-9 text
   wrongly asked of it. `W_repoint` is normative for the first job and irrelevant to the second; the
   adoption CAS's safety is the GC ordering alone. The invariant remains: **no fragment is destroyed
   before its reclamation is durable in metadata**, and **no fragment is written after its evidence
   may have been reclaimed**.
   **A mark whose position never receives a fragment still needs a reclaimer (iteration-11
   finding 6).** Teardown deliberately marks a failed attempt's **full planned placement** (decision
   5), so a fragment authorized before the fence is covered wherever it lands — which means a
   position that never received one is marked too, by design. But today's GC consumes an `orphan:`
   key only while iterating an actual `list_fragments()` result, so a mark with no fragment beneath
   it is **never visited and never deleted**. `orphan:` is one of the few namespaces read by a
   *global* scan (`orphan_leases`), so repeated partial uploads would grow it without bound and
   eventually fail that scan at `SCAN_CAP` — halting GC for the whole deployment, the F7-class
   outcome this protocol exists to avoid. GC therefore also reclaims **the marks themselves**: the
   positions in the ledger that no server's `list_fragments()` reported are exactly the
   fragment-less marks, computable in the pass GC already runs, and one older than the
   **late-write deadline** is deleted in the sweep's ordinary `B`-batches — **provided the ABSENCE
   was observed after that deadline, not merely the mark aged past it (iteration-13 finding 5).**
   A pass that listed a server *before* the deadline holds a listing that a fragment may land into
   afterwards while still inside its authorized window; treating that stale listing as proof of
   absence deletes the evidence for a fragment that is about to exist, leaving it unreferenced and
   unevidenced — outcome (a), reached through the very sweep added to bound the ledger. So the
   deletion is conditioned on the *observation*, not the mark: either the listing that showed the
   position empty was taken after `orphaned_at + W_repoint + W_write + δ_clock`, or the sweep makes
   a **fresh targeted existence check** at delete time. A pass that cannot satisfy either simply
   leaves the mark for the next pass, which is the safe direction and costs one more cycle.

   **That deadline is `W_repoint + W_write + δ_clock`, not `W_write + δ_clock` (iteration-12
   finding 3).** A repoint **pre-mark is legitimately fragment-less** for up to `W_repoint` — that
   is the whole point of pre-marking *before* writing the destination — so sweeping at the shorter
   deadline would delete a pre-mark out from under a worker still inside its own budget, which then
   passes its age check, authorizes the write, and strands it with no reference and no evidence.
   The two rules were added in the same round and did not compose. The sweep therefore retains any
   fragment-less mark until `W_repoint + W_write + δ_clock`, still **strictly inside** `G_orphan`
   by the sizing above, so nothing is retained past its grace. (An implementation may tighten this
   per event kind — a teardown mark cannot be awaiting a repoint write — since the mark carries its
   event identity; the uniform bound is the safe default.)

   **And the ledger itself must be walked in pages, not one scan (iteration-12 finding 4).**
   `orphan_leases` reads the whole prefix with `scan` today (`gc.rs:315-330`), and this proposal
   makes that unsurvivable: a **single** maximum segmented-object retirement installs up to
   ~1.78 M `orphan:` records (decision 4's own fan-out number) against a `SCAN_CAP` of 1,048,576 —
   and `scan` fails whole, with no partial result. One large delete would take GC out permanently,
   and with it the fragment-less-mark cleanup that is supposed to bound this very namespace: the
   failure is **self-sealing**, which is the worst property a failure can have. `orphan:` therefore
   joins `retire:` as a namespace **never read by a single scan** — walked with `scan_page` in
   cursor-keyed pages (X70's contract, whose no-skip-for-stable-keys rule is what makes a page-wise
   sweep sound), naturally sub-ranged by the `<dserver>` component of the key so a pass can join a
   page against that server's `list_fragments()` result, with the sweep's mutations byte-budgeted
   per batch as everywhere else. Not a new mechanism: the one decision 4 already introduced for
   `retire:`, applied to the other namespace whose cardinality this protocol drives past the cap.

   So every mark this protocol writes has a deleter whether or not its position is ever written, and
   the `orphan:` namespace is **accessible at every cardinality** rather than bounded below a cap it
...
3. **Admission bounds what cannot be drained: the map, the part, the segment, the session's
   ...
   **Every refusal that happens AFTER chunks are staged must compensate, not just return
   (normative; iteration-11 finding 4).** The cumulative `MAX_STAGED_CHUNKS` check refuses at
   **commit** time, by which point the attempt has already written its owned `sidx:` entries, its
   fragments and its slot — and the session deliberately stays `Open` and usable. Returning
   `400 EntityTooLarge` and stopping there strands all of it until the whole session is aborted or
   reaped: the bytes stay protected, the `sidx:` records stay live, and the **slot is never
   released**, so a client that repeats the refusal — which the "leave the session usable" answer
   positively invites — fills `MAX_INFLIGHT_PARTS` and turns the usable session into a permanent
   `503`. So every post-staging local refusal routes through the **same live-session compensation
   batch** as a losing writer: `retire:bytes:{chunks}` for the staged bytes, the keyed
   `require(slot == prior)` release, and this attempt's owned `sidx:` deletes, all under
   `require(mpu == Open@E)`. The refusal is then a *bounded* event with an exit, not an
   accumulation — which is what "the session stays usable and abortable" has to mean.
...
| `MAX_INFLIGHT_PARTS` | `[1, min( MAX_PARTS_PER_SESSION , ⌊SCAN_CAP / (2·MAX_PART_CHUNKS)⌋ , ⌊(E_tx/2) / (bytes per slot key)⌋ , B_ops )]` — **four clamps** (the first added by iteration-13 finding 2: a session cannot have more parts in flight than it may ever hold, and the `W_ref/2` arithmetic below silently assumed it; `B_ops` is the operation-count cap of iteration-13 finding 3, since the fence pins one precondition per index) — **two clamps, both applied by the implementation** (iteration-10 finding 5) | owned `sidx:` per session `≤ MAX_INFLIGHT_PARTS × MAX_PART_CHUNKS ≤ SCAN_CAP/2` — **enforced by the `slot:<id>:` key space** (indices `[0, MAX_INFLIGHT_PARTS)`, each claimed by `require_absent`), residue counted (F11a), so the per-session teardown scan (and each per-session reference-build range) is always safe. Because the value defines the key space, **raising it is unconditionally safe; lowering it** leaves live sessions holding indices above the new cap until those parts finish — a transient over-cap bounded by the *old* value, never an unbounded one (a rollout note, not a correctness case)  **The second clamp is what keeps the whole-range batches legal**: the reaper's idle fence pins every slot index it observed (one precondition each) and the terminal delete removes every surviving slot, both in **one** batch — and the `SCAN_CAP` clamp alone permits values (≈524,288 at `MAX_PART_CHUNKS = 1`) whose enumeration blows the `E_tx = 10 MB` envelope, leaving an abandoned session permanently unfenceable *and* undeletable, holding its admission count forever. Neither batch can be split: the terminal delete is the exactly-once `count` decrement point, and the fence's pin is only sound if it covers the whole range it judged. So the bound is byte-derived, exactly as `B` is, and the two clamps compose the same way `MAX_SESSIONS`' do | #508 |
| `MAX_OWNED_FLEET` (fleet-wide in-flight owned entries) | `= MAX_SESSIONS × MAX_INFLIGHT_PARTS × MAX_PART_CHUNKS`, and since `MAX_SESSIONS = ⌊W_ref/U_ref⌋` while **either** branch of `U_ref` is `≥ 2 × MAX_INFLIGHT_PARTS × MAX_PART_CHUNKS` (the second charges that term twice by construction; the first does because `MAX_INFLIGHT_PARTS ≤ MAX_PARTS_PER_SESSION` is now a **clamp on the knob**, not an assumption — iteration-13 finding 2: without it, 10,000 parts against 20,000 in-flight slots at one chunk per part makes the raw branch 30,000 refs while owned entries take 20,000, i.e. two thirds of `W_ref`, not half), this is `≤ W_ref / 2` — at most half the reference budget, under both the raw and the ceiling-bound derivation | owned entries live under the disjoint `sidx:` prefix, so **no global `scan("pending:")` (restore/sweep) enumerates them** (finding 3); they are read only through per-session `sidx:<id>:` ranges each `≤ SCAN_CAP/2`, whose *sum* is already inside the `W_ref` charge above | derived (not a free knob) |
...
| `B` (drain / segment batch) | **`min(B_bytes, B_ops)`** — mutation bytes per commit `≤ E_tx/2 = 5 MB`, **and** an operation count `B_ops` calibrated to keep a batch's sequential in-transaction round trips inside the **5-second** half of the envelope on the slowest supported backend | one commit stays inside `E_tx = 10 MB / 5 s` on **both** axes. Bytes alone do not prove termination: TiKV awaits each `get_for_update`/`put`/`delete` sequentially (`metadata-tikv/src/lib.rs:1063-1095`) and FDB awaits every precondition read (`metadata-fdb/src/lib.rs:1425-1438`), so ~1,000 small marks can exceed 5 s while far under 5 MB — and a batch that always times out is stuck, not slow (iteration-13 finding 3) | #625 |
| `R_publish` (publish retries) | `[1, small]` | bounded so Complete terminates; exceeding it releases the fence rather than looping | #508 |
| `MAX_COMPLETE_ATTEMPTS` (fences per session) | `[1, small]` | each Complete fence mints a new `Completing@E` epoch whose rolled-back segments become one `retire:records:{seg:<g>:<E>}` obligation, so an unbounded fence/rollback cycle would let **one** session install obligations and `seg:` generations without bound (`W_session` alone does not bound it — a client can loop faster than the drain). The counter lives in the session record, which every fence CASes anyway; at the cap Complete is refused and the session's only exit is Abort | #508 |
...
| `W_repoint` (pre-mark → destination-write deadline) | `W_repoint > 0`, enforced fail-closed **before the worker authorizes the destination write** (it restarts from a fresh pre-mark instead), and **`G_orphan > W_repoint + W_write + δ_clock`** (strict) | It bounds a **caller-side action** — whether this worker issues the write — which is what a caller-side deadline can soundly do; it is **not** what makes the adoption CAS safe (that is GC's reclaim-intent ordering, X61), because no caller-side check can bound when a transaction *applies*. Without it, a worker that pauses between pre-mark and write can have its evidence reclaimed and then strand the fragment it writes (iteration-11 finding 7) | reconstruction + rebalance |
...
2. **Fast path — a live-session loser compensates; nothing else can make it lose
   ...
   - **its own slot record moved or vanished** while the session is still `Open@E` and the part key
     is unchanged — while a session is `Open` the writers of that key are this attempt (its
     release **and now its own liveness batches**, iteration-11 finding 2) and, *once released*,
     whichever later attempt claims the freed index; a reaper teardown cannot touch an `Open`
     session. **The branch therefore splits, and the split is load-bearing (iteration-12 finding
     1):** the committer re-reads the **part record** and
     - **finds its own commit present** ⇒ this is a retry of a batch that in fact committed
       (X38's re-read case): return success without compensating, and never re-release the slot,
       so no double release is representable (iteration-7 finding 4);
     - **finds it absent or another attempt's** ⇒ the slot moved for the *other* reason it now can
       — **this request's own renewal rewrote it** between the commit's read and its commit — so
       there is no landed commit to report and treating the branch as one would return success for
       a part that does not exist. It is the **benign retry** of decision 5: re-read the slot,
       rebuild the batch, commit again. (Quiescing the renewal loop before building the batch makes
       this unreachable in a conforming implementation; the branch exists so that a
...
- **The request quiesces its own renewal before committing the part (normative; iteration-11
  finding 3).** Once every liveness batch rewrites the slot (above), this request's *own* renewal
  loop becomes a writer of the very key its part commit preconditions on
  (`require(slot:<id>:<k> == prior)`) — so a renewal landing between the commit batch's read and
  its commit produces "session still `Open`, part record unchanged, own slot moved" **without any
  prior release**, which is exactly the state the classification below treats as impossible. The
  request therefore **stops and joins its renewal loop before building the commit batch**; its
  final renewal stamps a lease covering at least the commit's transaction deadline, which is
  bounded by the `E_tx` envelope, so quiescing cannot let the lease lapse under the commit. And as
  a belt-and-braces classification rule: a commit that fails **only** on its own slot precondition,
  with the session still `Open@E` and the part record unchanged, is a **benign retry** — re-read
  the slot, rebuild, commit again — and never the losing-writer compensation, which would discard a
  part whose chunks are all durable. Without both, the protocol has a reachable state with neither a
  success response nor a bounded retry path.
...
- **Renewal is conditional but NOT atomic with its own expiry test, so the idle fence carries a
  ...
  **The fence therefore preconditions on the liveness evidence it read.** The idle arm's fence
  ...
  slot record is a per-request liveness witness that any renewal necessarily rewrites. **"The same
  loop" is not sufficient for that, and the requirement is atomicity (normative; iteration-10
  finding 2): every owned-`sidx:` lease renewal MUST rewrite its request's `slot:<id>:<k>` in the
  SAME metadata batch.** Renewing the staging leases in one batch and the slot in another leaves a
  window where the reaper has read both as expired, the `sidx:` renewal commits, and the slot bytes
  the fence pinned are still unchanged — so the fence commits and aborts a request that has
  demonstrably just made progress, which is precisely the hole the pin was added to close. One
  batch per renewal round makes the slot rewrite un-skippable, and costs nothing: the renewal
  already commits a batch, and the slot is one more small key in it. **The same requirement binds
  the `sidx:` INTENT batch, not only renewals (iteration-11 finding 2):** an intent creates a
  *fresh, live* owned lease, which is progress by any reading, yet it changes no slot bytes — so a
  reaper that read an expired slot could fence a part that has just staged a new chunk, with both
  the session and the slot preconditions still passing. Every batch that creates or renews this
  request's liveness evidence therefore rewrites its `slot:<id>:<k>` atomically with it. The slot
  is the request's **own** key, so none of this contends with anything. A renewal,
...
- **Abandonment rule (two arms).** A session in `Open` is fenced to `Aborting` when **either**:
  ...
  (`Completing → Open`) — **unless they are over-age, in which case the residency ceiling applies
  to them directly (iteration-10 finding 7).** `W_session` is measured from `created_at_millis`, so
  evaluating it only for `Open` left a session that Completes just before its deadline holding its
  staged references for up to a further `W_completing` (rollback, then a later pass to reap) — which
  makes the F14 residency promise, and the F6 drain bound that is *derived* from it, false by that
  margin. An over-age `Completing` session therefore takes `Completing@E → Aborting@E+1` **directly**,
  in one batch that also installs `retire:records:{seg:<g>:<E>}` for the segments that attempt
  wrote (the same shape as the restore fence, X57) — never a rollback first, which would spend
  another pass and re-open a session the ceiling has already condemned — rollback is always safe because the epoch bump
  ...
  X57) **and to the over-age ceiling above and the operator abort (FU-6)** — the reaper reaches it
  only through the ceiling, never through `W_completing`: a restore has declared every open session dead, so there is no later
  attempt to preserve.
...
- **The clock lifecycle and its owner (F10, **D-E**).** The abandonment judgment reads the
  ...
  **skips (and alarms on) any session whose `clock_source` it does not own — before it reads that
  ...
  evaluated except under an owned session).

  **But inheritance is only transitive if every WRITER of a session-scoped stamp shares that
  source, and that needs stating rather than assuming (normative; iteration-14 finding 1).** The
  guard is read once, from the record the *creating* gateway wrote — while `slot:` `reserved_at`,
  `psum:` `committed_at`, and the `Completing` `fenced_at` are stamped by whichever gateway handled
  that later request, which during a rolling configuration change need not be the creator. The
  reaper then validates the session's source and proceeds to trust stamps that never came from it:
  a future-epoch progress stamp defers cleanup indefinitely, and subtracting against an older epoch
  reaps a live upload immediately — the #557 defect class re-entered through the back door, after
  the front door was locked. So **every operation that writes a session-scoped timestamp MUST first
  verify that its own clock source equals the session record's `clock_source`, and refuse
  (`503`, with the same alarm the reaper raises) if it does not.** The check is one comparison
  against a field the operation already reads for its fence precondition, so it costs nothing; a
  single field on the session stays sufficient, because the invariant it now asserts — *every stamp
  in this session was written under this source* — is enforced at each write rather than hoped for.
  The alternative, tagging each stamp with its own source and validating per-stamp, is strictly
  more machinery for the same guarantee, and is recorded as rejected. The fail direction is safe (skew → a false-positive
...
- **Admission control — the enforceable, exact bound (**D-C**, F7/F12).** `CreateMultipartUpload`
  ...
  discipline as steady state, and the counter never double-initializes. **An *unknown* create
  outcome is recovered with the ORIGINAL upload id, never a fresh one (iteration-10 finding 6).**
  This is the slot-reserve rule (X68) one record up, and for the same reason: when the create batch
  returns `CommitUnknownResult` with `may_still_commit == true`, an absent re-read does not prove
  the session record and its `count` increment will not land afterwards
  (`crates/traits/src/lib.rs:241-247`). Erroring out, or retrying under a **new** upload id, can
  therefore leave an `Open` session no client will ever name — unreachable, holding an admission
  slot until the reaper's `W_session` ceiling — and a run of metadata timeouts can exhaust
  `MAX_SESSIONS` that way. So the gateway re-drives the **same upload id** — but
  **rebuilds the `mpuctl` precondition from a fresh read (iteration-13 finding 8)**. Replaying the
  *identical* batch would be futile whenever any other Create or terminal delete moved the
  admission record in the meantime: its counter CAS is stale, so it can never commit however many
  times it is driven, even with the upload id still free and capacity available. The mutual
  exclusion that makes the retry safe comes from `require_absent(mpu:<id>)` alone — the id, not the
  whole batch — so the retry re-reads `mpuctl`, re-checks `count < max_sessions` and the profile,
  and increments from the value it just read. At most one attempt lands, and either outcome is the
  same observable session, which the client can then use. It re-reads to distinguish
  landed (proceed) from definitively-not-landed (retry or refuse), and with
  `may_still_commit == false` a single re-read settles it as usual. The terminal delete CASes
...
- **The governing limit is stored with the counter, not derived per gateway (iteration-9
  ...
  **`{ count, max_sessions }`**, one record, and:
  ...
  - **the record stores the budget *profile*, not only its quotient (iteration-10 finding 4).**
    Comparing the derived integer alone does not detect every unsafe transition: different
    `(W_ref, U_ref)` pairs collapse to the same `⌊W_ref / U_ref⌋`, so a custodian host with a
    *smaller* `W_ref` can agree on `max_sessions` with gateways still admitting sessions whose
    per-session footprint was derived under the old, larger caps — the equality check passes and
    aggregate reference construction still overruns the new host's RAM. So `mpuctl` carries
    `{ count, max_sessions, profile }` where `profile` is the tuple that *establishes* `U_ref` —
    `W_ref`, `MAX_PART_CHUNKS`, `MAX_PARTS_PER_SESSION`, `MAX_INFLIGHT_PARTS`, `MAX_STAGED_CHUNKS`
    — and admission compares the **profile**, not the quotient;
  - **the custodian validates the profile before building the reference set, not only gateways
    (iteration-13 finding 7).** Checking admitters alone protects the wrong process: the component
    that actually holds `W_ref` worth of chunk-refs in memory is the **reconcile pass**, and during
    a rolling change a new custodian can have a smaller `W_ref` while sessions admitted under the
    old profile are still live and old gateways are still legitimately admitting against the
    ledger's value. So the custodian/reaper compares **its** local profile with `prior.profile` and
    **fails closed with an alarm** before constructing the staged reference set, rather than
    discovering the mismatch as an OOM. The operator CAS gated on `count == 0` cannot substitute:
    it constrains the ledger, not what any individual process is configured with. `mpuctl` is
    therefore read by the custodian as well as by Create, and §1's record table says so;
  - a gateway whose local profile **differs** from `prior.profile` **refuses to admit and alarms**
    rather than silently deferring to either value — a fail-closed configuration-skew signal, since
    the disagreement means one of the two hosts is sized for a budget it is not getting (the same
    "misconfiguration is visible, not absorbed" stance as the reaper-absent refusal in the
    implementation order). **A profile change is gated on the live population draining**: because
    sessions already admitted carry the *old* profile's footprint, the operator CAS that installs a
    new profile is admissible only while `count == 0`, or is staged as refuse-new-admissions-then-swap
    — one of the two, never a hot swap under live sessions;
  ...
  `require_absent(mpuctl)` with `{ count: 1, max_sessions, profile }`, so a fresh store
...
| `seg:` | `≤ MAX_ROOT_SEGMENTS` per **attempt** (epoch-scoped, decision 7) | per-object range `scan("seg:<g>:<epoch>:")`, the epoch read from the root |
...
one reaper pass (dispatched from the fenced control point, like every custodian loop):
  ...
  2. for each session, in id order:               # deterministic order, resumable
       ...
       if not foreign and state == Completing:
             # the residency ceiling is measured from CREATION and applies here too, or the
             # W_session promise (and the drain bound that rests on it) is false by up to
             # W_completing (iteration-10 finding 7). Over-age goes STRAIGHT to teardown.
             if now - created_at > W_session:
                   CAS Completing@E -> Aborting@E+1 + put retire:bytes:{session, parts}
                                                     + put retire:records:{seg:<g>:E}
             elif now - fenced_at > W_completing:
                   CAS Completing@E -> Open@E+1 + put retire:records:{seg:<g>:E}  # rollback;
                                                                                   #  only THIS epoch
  ...
  3. for page in scan_page("retire:", cursor, limit):   # paginated seam; each page < SCAN_CAP
       drain up to E_tx/2 BYTES of mutations, then commit; repeat next range / next pass
         ...
         retire:records: delete this batch of the records the payload names (parts and/or
                         seg:<g>:<epoch>) -> when all deleted: delete the obligation (never
                         orphan-mark)
```
...
`SCAN_CAP` by an enforced bound; `retire:` **and `orphan:`** are walked in cursor-keyed bounded ranges (the latter added by iteration-12 finding 4 — a single maximum segmented-object retirement installs ~1.78 M marks, well past `SCAN_CAP`); **no global
...
| **Clock guard read from the owned entry, not the session (finding 6b).** Reap using an owned-lease stamp before checking the session's `clock_source` | Write a session with a foreign `clock_source` and an owned entry with a *local*-looking lease; the reaper MUST read the guard from `mpu:` **first** and MUST NOT evaluate that owned lease in any timestamp-comparing judgment. It MUST NOT, however, skip the session's **clock-free** work — the `sidx:` walk, the `retire:` drain and (for `Aborting`) the terminal delete still run, per X71/X82; asserting a whole-session skip here would license the absorbing state those closed. |
...
| **Clock-epoch mismatch — skip the *judgments*, never the pass (iteration-11 finding 1)** | Write a session record with a foreign `clock_source`: the reaper MUST raise an operator signal and MUST NOT evaluate any timestamp-comparing arm (`W_open`, `W_session`, `W_completing`, `W_tombstone`) against it. But assert the converse too, or the row licenses the absorbing state X71 closed: a foreign-clocked session that is already `Aborting` or `Completed` MUST still have its owned `sidx:` range walked, its `retire:` obligations drained, and (for `Aborting`) its terminal delete fire — none of which reads a session timestamp. A `Completed` foreign-clocked session holds only until FU-6's operator-authorized terminal expiry. An implementation that skips the whole session strands the records and the admission slot forever. |
...
segment-group key `seg:<group-nonce>:<epoch>:<i>` and its crash-evidence/rollback machinery (§c).
...
record `seg:<group-nonce>:<epoch>:<i>` (§1). The **segment-group nonce** `group` is the minting
...
then the bounded range `scan("seg:<group-nonce>:<epoch>:")` (≤ `MAX_ROOT_SEGMENTS`), the epoch
...
`Completing@E` and validating (decision 1.2), takes its segment-group nonce `group = (<upload-id>,
...
MAX_ROOT_SEGMENTS` segments. It writes the `seg:<group-nonce>:<E>:<i>` records in **byte-budgeted**
...
`seg:<group-nonce>:<E>:*` records whose bytes are **still protected by the `part:` records** (the
...
- **Same-gateway recovery** (the completer received `CommitUnknownResult` and re-reads its own
  ...
  (`seg:<group-nonce>:<E>:<i>`) and their content are fixed by the frozen part set at that
  epoch** — then flips.
- **Rollback / abort** (reaper after `W_completing`, or a rejected Complete): the transition
  installs `retire:records:{seg:<group-nonce>:<E>}`, naming **exactly epoch `E`'s** segment keys,
  ...
  and writes the **disjoint** range `seg:<group-nonce>:<E'>:*` (the part set may have changed;
...
by the epoch-scoped `retire:records:{seg:<g>:<E>}` on rollback) and **bounded** (≤
...
drained `retire:records:{seg:<g>:<E>}` obligation under the ordinary `retire:` bounded-range +
...
`Segmented`, the bounded range `scan("seg:<group-nonce>:<epoch>:")` (≤ `MAX_ROOT_SEGMENTS`), the epoch
...
its `seg:<group-nonce>:<epoch>:*` records in byte-budgeted batches. The committed inode carried the
...
the drain re-reads each `seg:<group-nonce>:<epoch>:<i>` record's **current** `ChunkRef.placement` at
...
root (pointing at `group = (id, E)`) and is midway through reading `seg:<g>:<E>:*`. Rather than add
...
`retire:records:{seg:<g>:<E>}`):
- A consumer resolving a segmented map reads the root's `group`, then the bounded range
  `scan("seg:<g>:<E>:")`. If a segment read returns **absent**, it **re-reads the root**. A root
...
| **Scope segment keys per-session, not per-attempt, so a stale rollback obligation deletes a later attempt's published segments (the iteration-2 F18 refutation, outcome c)** | DST: fence `Completing@E`, write segments, roll back (installing `retire:records:{seg:<g>:<E>}`), re-Complete to `Completing@E'`, publish, and drain the *still-pending* epoch-`E` obligation **after** publication; assert the **published** object's `seg:<g>:<E'>:*` records survive and the object resolves. Per-attempt (epoch) keys make the ranges disjoint; a session-scoped key deletes the live segments (X40). |
| Leave dangling `seg:` records after a completer crash (F18) | Kill a completer between segment writes and the flip, then roll the session back: assert every `seg:<g>:<E>:` record of that epoch is reclaimed by `retire:records:{seg:<g>:<E>}` and **no** fragment is orphaned (the parts still protect them). |
...
| Resolve a segmented map with a global `seg:` scan | Publish enough segmented objects that a global `seg:` scan would cross `SCAN_CAP`; assert `reconcile_step` still succeeds — resolution MUST be per-object bounded `seg:<g>:<epoch>:` ranges. |
| Orphan-mark a superseded segmented generation's fragments but leak its `seg:` records | Overwrite a segmented object; after the drain, assert no `seg:<g>:<epoch>:` record of the prior generation survives and its fragments are orphan-marked. |
...
| Fragment-write deadline | `W_write` — the fail-closed timeout on a fragment write to a D server (the rubric's *await discipline*, `AGENTS.md:181-183`) | write path **and the D server** | **both ends, and the table must say so (iteration-13 finding 6)**: the caller's fail-closed await abandons the write past the deadline, **and the D server refuses to apply a write whose deadline has passed** — a caller timeout does not un-queue a request the server has already accepted, so a caller-only rule leaves the server free to apply it after the orphan evidence's grace has elapsed. The knob table and the seeded regression already require both sites; this row described only the first | the same deployment wall clock; **coupled to orphan grace by the strict `G_orphan > W_write + δ_clock`** (`δ_clock` = the clock's resolution/skew margin) so no authorized fragment can land at or after its landing position's `orphan:` evidence is reclaimable (finding 3) |
...
| X37 (F18) | Completer dies between segment writes and the root flip | Segments are evidenced (`publish_target` while `Completing@E`) and the fragments stay part-protected; the owning gateway's same-epoch recovery re-runs idempotently (deterministic `seg:<g>:<E>:` keys) or a rollback installs `retire:records:{seg:<g>:<E>}` naming only epoch `E` | D7 |
...
| X40 (F18) | Rollback → re-Complete → **publish while the prior attempt's `retire:records:{seg}` obligation is still pending**, then that obligation drains | The stale obligation names epoch-`E` keys `seg:<g>:<E>:*`; the re-Complete wrote a **new** epoch `seg:<g>:<E'>:*` and published `group = (<id>, E')`; draining the stale obligation deletes only epoch-`E` keys, so the published object's epoch-`E'` segments survive — no published object loses its map (the iteration-2 F18 refutation, closed by per-attempt scoping) | D7 (F18) |
...
| X57 (iteration-7 adversary) | A session is snapshotted in `Completing@E` **after** its segment-write phase (`seg:<g>:<E>:*` durable) but **before** its root flip; the image is restored and the restore pass fences it | The **restore-fence transition** `Completing@E → Aborting@E+1` (§2, §3) commits, in one batch, the state change, `retire:bytes:{session, parts}` **and** `retire:records:{seg:<g>:<E>}` — so that attempt's segments have a named deleter. Fencing it through the `Open` abort row instead (which the pre-iteration-7 text implied, since no `Completing → Aborting` edge existed) leaves those `seg:` records unreachable by **every** deleter the design defines: no root flip ⇒ no committed inode to supersede, and no rollback ⇒ no `retire:records:{seg}` obligation. Their fragments are still orphan-marked by `retire:bytes:{session}`, so this is record residue rather than lost bytes — but it is unbounded residue in a namespace the design otherwise bounds, and it was absent from the register | D-B, D2 (restore), D3, D7 |
...
| X61 (iteration-9 finding 1, **corrected by iteration-10 finding 1**) | A repoint's adoption CAS applies after GC has deleted the destination fragment but before GC's key cleanup commits | The ledger key outlives the bytes: GC deletes the fragment inside its per-server loop and commits the matching `orphan:` delete only after the whole sweep (`gc.rs:189-207`), so `require(orphan:<P_new> == prior)` can pass over deleted bytes. The iteration-9 answer — refuse if the pre-mark is older than `W_repoint` — was **also wrong**, and wrong in the same way as X62: it is evaluated in the caller before the commit, so a delayed transaction (or a `CommitUnknownResult` replay) can still apply late. Closed by ordering GC's own work: it **CASes `orphan:<pos>` to `reclaiming` and commits that batch *before* `delete_fragment`**, so any adoption preconditioned on the original bytes fails from the instant reclamation starts — no clock on either side. `W_repoint` is demoted to an advisory residency bound | D4.2, D2 (repoint/re-place), GC ordering |
...
| X74 (iteration-10 finding 2) | The reaper reads a request's `slot:` and `sidx:` leases as expired; the request's **`sidx:` renewal** commits, but its slot rewrite has not yet | The iteration-9 pin said the half-TTL loop renews "the slot lease together with" the owned leases — but "the same loop" is not atomicity. Split across two batches, the slot bytes the fence pinned are still unchanged when the `sidx:` renewal lands, so the fence commits and aborts a request that has demonstrably just progressed. Closed by requiring the slot rewrite in the **same metadata batch** as every owned-lease renewal, which costs one small key in a batch the renewal already commits; pinning the owned entries instead would be `≤ MAX_INFLIGHT_PARTS × MAX_PART_CHUNKS` preconditions | D6, D5, X62 |
| X75 (iteration-10 finding 3) | An upgraded custodian meets an `orphan:` mark written by an older build | Orphan values are bare decimal `orphaned_at_millis` today (`gc.rs:110-122`, parsed at `:315-330`), so the event identity the three-arm rule needs is a **value-schema change with legacy data in the field**. Decoding is dual-format: a bare decimal is a legacy mark with **no** identity, which therefore takes the *different-identity* arm and is replaced with a structured value and a fresh stamp by the first unreference event that meets it — self-limiting (the replacement writes an identity, so the co-drainer of that event then skips) and needing no migration pass. A reader that only reads never rewrites, so no grace clock restarts; a downgraded custodian fails the bare-`u64` parse and drops the entry, leaving the fragment unreclaimable rather than wrongly reclaimed | D4.2, X63, Backward compatibility |
| X76 (iteration-10 finding 4) | Two hosts derive the **same** `MAX_SESSIONS` from **different** `(W_ref, U_ref)` pairs during a rolling change | Equal quotients hide unequal footprints: a custodian with a smaller `W_ref` can agree on `max_sessions` with gateways still admitting sessions sized by the old, larger caps, so the equality check passes and aggregate reference construction still overruns the new host's RAM. `mpuctl` therefore stores the **profile** that establishes `U_ref` — `W_ref`, `MAX_PART_CHUNKS`, `MAX_PARTS_PER_SESSION`, `MAX_INFLIGHT_PARTS`, `MAX_STAGED_CHUNKS` — and admission compares that, not the quotient; a profile change is an operator CAS gated on the live population having drained, since already-admitted sessions carry the old footprint | D6 (**D-C**), D2 (`W_ref`), X64 |
| X77 (iteration-10 finding 5) | A deployment picks a legal `MAX_INFLIGHT_PARTS` near the `SCAN_CAP` clamp (≈524,288 at `MAX_PART_CHUNKS = 1`) and a session is then abandoned | The idle fence pins **every** slot index it observed and the terminal delete removes **every** surviving slot, each in *one* batch — and neither can be split (the delete is the exactly-once `count` decrement point; the pin is sound only over the whole range it judged). At those legal values both batches exceed the `E_tx = 10 MB` envelope, so the session is permanently unfenceable *and* undeletable and holds its admission count forever. Closed by a second, **byte-derived** clamp on the knob — `⌊(E_tx/2)/(bytes per slot key)⌋` — composed with the `SCAN_CAP` one exactly as `MAX_SESSIONS`' two clamps compose | D5 (`slot:`), D6, §3 |
| X78 (iteration-10 finding 6) | `CreateMultipartUpload`'s batch returns `CommitUnknownResult` with `may_still_commit == true` and the immediate re-read finds nothing | Absence proves nothing (`traits/src/lib.rs:241-247`), so erroring out or retrying under a **new** upload id can leave an `Open` session no client will ever name — unreachable, holding an admission slot until `W_session`, and a run of timeouts exhausts `MAX_SESSIONS` that way. The gateway re-drives the **same** upload id with the **same** batch: `require_absent(mpu:<id>)` makes them mutually exclusive, so at most one lands and either outcome is the same usable session. X68's rule, one record up | D6, X68 |
| X79 (iteration-10 finding 7) | A session Completes just **before** its `W_session` deadline and its completer crashes | The ceiling was evaluated only for `Open`, so the session waited out `W_completing`, rolled back to `Open`, and needed a further pass to be reaped — holding staged references, and blocking a drain, for up to `W_completing` **beyond** the promised ceiling. That falsifies F14's residency claim and the F6 drain bound derived from it. The ceiling is measured from `created_at_millis` and now applies to `Completing` too: an over-age `Completing` session takes `Completing@E → Aborting@E+1` **directly**, in one batch that also installs `retire:records:{seg:<g>:<E>}`, never a rollback first | D6, D3, D-A, F6/F14 |
| X80 (iteration-10 finding 8) | An operator aborts a `Completing` session (FU-6) that has already written segments | The verb crosses the identical edge as the restore fence, so it needs the identical batch: without `retire:records:{seg:<g>:<E>}` installed atomically with the fence, that attempt's `seg:` records have no adopting inode (no root flip) and no rollback obligation naming them — the X57 hole reached through a different door. FU-6's contract now names the obligation | FU-6, X57, D7 |
| X81 (iteration-10 finding 9) | A client reuses a **completed** upload id inside the tombstone window with a **different** part list | The window returned the recorded `200` + ETag on the upload id alone, so the client is told *its* assembly succeeded while the stored object holds the earlier one — a silent wrong answer, worse than any error. Idempotence is keyed on **request identity**: the `Completed` record carries `complete_fingerprint` (a digest over the ordered `(part_number, digest)` pairs the winning Complete named), the tombstone answers `200` only on a match, and a mismatch is `404 NoSuchUpload` — what the same request receives one tick after the window closes anyway | D3, §2 (verb table) |
| X82 (iteration-11 finding 1) | An implementer follows decision 6's *failure-mode row* rather than its algorithm and skips a foreign-clocked session wholesale | The row still said "the reaper MUST skip it" after X71 restricted the guard to timestamp-comparing judgments — and the row is the testable artefact, so it would have been implemented. A foreign-clocked `Aborting`/`Completed` session would then never walk its `sidx:` range or reach its terminal delete, holding its admission slot forever, which is exactly the absorbing state X71 closed. The row now asserts **both** directions: no timestamp arm evaluated, **and** the clock-free teardown still runs | D6 (F10), X71 |
| X83 (iteration-11 finding 2) | A part stages a **new chunk** (a fresh, live `sidx:` lease) after the reaper read its slot as expired, before the idle fence commits | An intent creates liveness evidence without touching the slot, so the fence's session *and* slot preconditions both still pass and it aborts a demonstrably progressing part — X62's hole reached through creation rather than renewal. Closed by the same rule: **every batch that creates or renews this request's liveness evidence rewrites its `slot:<id>:<k>` atomically with it**, intents included. The slot is the request's own key, so nothing contends | D5, D6, X62, X74 |
| X84 (iteration-11 finding 3) | This request's **own** renewal rewrites its slot between the part-commit batch's read and its commit | Once every liveness batch rewrites the slot, the request's renewal loop becomes a writer of the key its own commit preconditions on — producing "session `Open`, part unchanged, own slot moved" with **no prior release**, a state the commit classification treats as unreachable, so the protocol has neither a success response nor a bounded retry. Closed twice over: the request **quiesces and joins its renewal loop before building the commit batch** (its last renewal covering the `E_tx`-bounded commit deadline, so nothing lapses underneath), and a commit failing **only** its own slot precondition with the session still `Open@E` and the part unchanged is classified a **benign retry**, never the losing-writer compensation that would discard durable chunks | D5, X74, X83 |
| X85 (iteration-11 finding 4) | A fully staged part is refused at commit by the cumulative `MAX_STAGED_CHUNKS` check | The refusal left the session `Open` and usable by design, but the only defined live-session compensation was for a same-part losing writer — so the refused attempt kept its `sidx:` records, its bytes **and its slot**. A client repeating the refusal (which "the session stays usable" invites) fills `MAX_INFLIGHT_PARTS` and turns that session into a permanent `503` with its bytes still protected. Every **post-staging local refusal** now routes through the same compensation batch as a losing writer — `retire:bytes:{chunks}`, the keyed slot release, its own `sidx:` deletes, under `require(mpu == Open@E)` — so a refusal is a bounded event with an exit | D4.4, D5 |
| X86 (iteration-11 finding 5) | A crash lands between GC's `reclaiming` CAS and its `delete_fragment` | The ordering X61 introduced deliberately opens this window, so it must be specified rather than assumed away: a durable `reclaiming` value with the fragment still present. `reclaiming` is therefore a **third value variant** (`{ orphaned_at_millis, event, reclaiming: true }`) that `orphan_leases` **decodes** — rejecting it would abort the GC pass at its decode boundary, skipping it would make the fragment permanently unreclaimable — and reads as "reclamation already decided; resume it": re-issue the idempotent `delete_fragment`, then delete the key, with no re-evaluation of a grace test that already elapsed | D4.2, X61, X75 |
| X87 (iteration-11 finding 6) | A failed attempt's teardown marks planned positions where **no fragment was ever written** | Marking the full planned placement is deliberate (it covers a straggler wherever it lands), but GC consumes an `orphan:` key only while iterating an actual `list_fragments()` result, so a mark with nothing beneath it is never visited and never deleted. `orphan:` is read by a **global** scan, so repeated partial uploads grow it past `SCAN_CAP` and halt GC deployment-wide — the F7 outcome by another route. GC now reclaims the marks themselves: positions in `orphan_leases` that no `list_fragments()` reported, once older than the late-write deadline **`W_repoint + W_write + δ_clock`** (X91 — a repoint pre-mark is legitimately fragment-less for `W_repoint`; retained before it, since that is the straggler window) **and only when the absence itself was observed after that deadline** (X96), deleted in the ordinary `B`-batches | D4.2, D5, X49 |
| X88 (iteration-11 finding 7) | A repoint worker pauses **between** writing its pre-mark and authorizing the destination write, long enough for GC to reclaim the mark | GC ages the mark through `G_orphan`, CASes it `reclaiming`, deletes nothing (no fragment yet) and removes the mark; the resumed worker's write then lands with **no reference and no evidence** (outcome (a)). `W_write` cannot cover it — it bounds only the interval *after* authorization. The worker therefore **MUST NOT authorize the write if its own pre-mark is older than `W_repoint`**, with **`G_orphan > W_repoint + W_write + δ_clock`** strict. This is a caller-side deadline gating a **caller-side action**, which is sound — unlike the iteration-9 attempt to use one to bound when a *transaction applies* | D4.2, X61, X49 |
| X89 (iteration-11 finding 8) | The bootstrap Create writes `mpuctl = { count: 1, max_sessions }`, omitting the schema's `profile` | Structural violations fail decoding rather than acquiring defaults ([ADR-0045][a45]), so the next Create or terminal decrement could not read or CAS the record — multipart admission wedged immediately after the very first session, on every fresh or upgraded store. The bootstrap shape is `{ count: 1, max_sessions, profile }`, matching the schema everywhere else. A one-field omission with a total-outage blast radius, and the same class of table-versus-prose drift the previous round's sweep was meant to catch | §1, D6, X76 |
| X90 (iteration-12 finding 4) | One maximum segmented-object delete installs ~1.78 M `orphan:` marks and GC then runs | `orphan_leases` reads the whole prefix with a single `scan` (`gc.rs:315-330`) against `SCAN_CAP = 1,048,576`, and `scan` fails **whole**, with no partial result — so that one delete takes GC out permanently, **including the fragment-less-mark cleanup that is supposed to bound this very namespace**. The failure is self-sealing: the mechanism that would recover it is the mechanism it disables. `orphan:` therefore joins `retire:` as a namespace never read by a single scan — `scan_page` in cursor-keyed pages, sub-ranged by the key's `<dserver>` component so a page joins against that server's `list_fragments()`, mutations byte-budgeted per batch. The fan-out number is decision 4's own, so the cap was always exceeded; this proposal is what made it load-bearing | D4.2, D4, X70, traits seam |
| X91 (iteration-12 finding 3) | GC's fragment-less-mark sweep meets a repoint **pre-mark** whose worker has not yet authorized its write | A pre-mark is *legitimately* fragment-less for up to `W_repoint` — that is what pre-marking before writing means — so sweeping at `W_write + δ_clock` deletes it out from under a worker still inside its budget; the worker then passes its own age check, authorizes the write, and strands it with no reference and no evidence (X88's hole, reopened by X87's sweep). The two rules landed in the same round and did not compose. The sweep's deadline is `W_repoint + W_write + δ_clock`, still strictly inside `G_orphan`; an implementation may tighten it per event kind, since the mark carries its identity | X87, X88, D4.2 |
| X92 (iteration-12 finding 5) | The last reference to a fragment carrying a **stale, already-expired** mark is removed, and GC runs before the drain refreshes that mark | The refresh is the drain's work and the drain is asynchronous, so GC sees unreferenced + expired, CASes `reclaiming`, and deletes — a GET overlapping the deletion gets none of the promised grace. X63's fix, relocated into its own gap. Serializing the refresh into the unreference batch is unavailable (that fan-out is precisely what `retire:` keeps out of it), so the **obligation itself becomes a protection class**: `protects()` is true for every position named by an installed, undrained `retire:bytes:` obligation. The stale mark is unactionable until the drain refreshes it, after which the fresh grace governs. Cost: reclamation waits for the drain — the direction this design accepts everywhere | D4.2, D2, X63 |
| X93 (iteration-12 finding 1) | This request's own renewal moves its slot mid-commit, and the commit's classification reaches the "slot moved while `Open`" branch | X84 added the benign-retry rule but left decision 5's *classification* saying that branch can only mean this attempt's release already landed — so an implementation following it would re-read the part record, find nothing, and have no defined outcome (or worse, report success for a part that does not exist). The branch now splits on that re-read: own commit present ⇒ the X38 retry-of-a-committed-batch case, return success without re-releasing; absent or another attempt's ⇒ the **benign retry**. Quiescing makes the second arm unreachable in a conforming implementation; it exists so a non-conforming one degrades to a retry rather than a false success | D5, X84, X38 |
| X94 (iteration-12 finding 2) | The classification-sweep test helper is run against a correct execution mid-handoff | "Exactly one class" is the wrong assertion and would fail on correct executions: this protocol **deliberately** overlaps protection across both handoffs — a published chunk is committed-referenced *and* still named by a staged `part:` record until `retire:records:{parts}` drains, and a retirement's marks precede its record deletions by construction. That overlap is what closes the gaps the source-before-destination orders exist to prevent (X67), so a disjointness assertion would pressure an implementer to remove the property that makes the handoffs safe. Invariant (2) is a **no-gaps** claim, not a partition: assert **at least one** safe class, and separately reject only genuinely incompatible pairs | Tests, D2, X67 |
| X95 (iteration-12 finding 6) | An implementer follows decision 6's *other* foreign-clock failure-mode row (the finding-6b guard-ordering row) | A second row still said the reaper "MUST skip the whole session", so X82's correction of the first row left the contradiction intact one row down — and the rows are the artefacts tests are written from. Both rows now assert the same two-sided property: no timestamp-comparing judgment against a foreign-clocked session, **and** its clock-free teardown still runs. Two rows saying different things about the same guard is the drift class these rounds keep surfacing | D6, X71, X82 |
| X96 (iteration-13 finding 5) | GC lists a D server, a fragment lands into that position afterwards but still inside its authorized deadline, and the same pass then judges the mark old enough to sweep | The listing is a **stale observation**, not proof of absence: treating it as proof deletes the evidence for a fragment that is about to exist, leaving it unreferenced and unevidenced — outcome (a), reached through the sweep added to bound the ledger. The deletion is therefore conditioned on the **observation**, not the mark: either the listing showing the position empty was taken after `orphaned_at + W_repoint + W_write + δ_clock`, or the sweep makes a fresh targeted existence check at delete time; a pass that can do neither leaves the mark for the next pass | X87, X91, D4.2 |
| X97 (iteration-13 finding 4) | The retirement drain falls behind under sustained deletes while GC needs to know which positions a pending obligation protects | X92 said `protects()` covers "every position named by an installed `retire:bytes:` obligation" — which requires **expanding** every pending obligation in memory, in the one namespace this design declines to bound by cardinality, with a single segmented generation expanding to ~1.78 M positions. The reconcile pass's memory becomes unbounded and GC OOMs or never finishes: a reader-safety window traded for a liveness failure of the whole collector. Pagination bounds each read, not the aggregate. The bounded form is a **keyed lookup** — an event's token *is* its obligation key, so the test is one `get` of `retire:bytes:<token(mark.event)>`, O(1) memory per position — and the residual stale-mark population (rollout skew, restore rewind) is **cleared once** by the upgrade and restore passes rather than protected against forever | X92, D4.2, D2 |
| X98 (iteration-13 finding 3) | A drain batch of ~1,000 small `orphan:` marks, or an idle fence pinning tens of thousands of slot indices, runs against TiKV or FDB | `B` was derived from **bytes alone**, but the envelope is `10 MB / 5 s` and every backend applies preconditions and mutations **sequentially inside the transaction** (`metadata-tikv/src/lib.rs:1063-1095`, `metadata-fdb/src/lib.rs:1425-1438`). So a batch far under 5 MB can exceed the 5-second deadline and time out **indefinitely** — and a batch that always times out is stuck, not slow, so the drain step, fence or terminal delete it carries never completes at all. `B = min(B_bytes, B_ops)`, and for the batches that cannot be split (the terminal delete, the fence's pin) `B_ops` becomes a clamp on `MAX_INFLIGHT_PARTS` rather than on the batch | §3, D5, X77 |
| X99 (iteration-13 finding 7) | A rolling change gives the **custodian** a smaller `W_ref` while old-profile sessions are live and old gateways still admit against the ledger | X76's profile check bound only admitters, which is the wrong process: the one that holds `W_ref` worth of chunk-refs is the **reconcile pass**. The custodian/reaper therefore compares its own profile with `prior.profile` and **fails closed with an alarm before constructing the reference set**, rather than discovering the mismatch as an OOM. The operator CAS gated on `count == 0` constrains the ledger, not any individual process's configuration, so it cannot substitute; `mpuctl` is now read by the custodian as well as by Create | D6 (**D-C**), X76, D2 |
| X100 (iteration-13 finding 6) | A caller's `W_write` await expires while the D server has already accepted or queued the fragment write | A caller timeout does not un-queue an accepted request, so caller-only enforcement leaves the server free to apply the write after the orphan evidence's grace has elapsed — the straggler X49 closes only if **both** ends enforce. The knob table and the seeded regression already required the D-server refusal; the authoritative clock-lifecycle table described a caller-only abandonment, and the clock table is what a reader treats as the enumeration. Both sites are now named there | Clock table, X49, D4.2 |
| X101 (iteration-13 finding 8) | An ambiguous Create is re-driven after another Create or terminal delete moved `mpuctl` | Replaying the *identical* batch can never commit — its counter CAS is stale — so the recovery would spin forever with the upload id free and capacity available. The mutual exclusion that makes the retry safe comes from `require_absent(mpu:<id>)`, the **id**, not the whole batch: the retry re-reads `mpuctl`, re-checks `count < max_sessions` and the profile, and increments from the freshly read value | D6, X78 |
| X102 (iteration-13 finding 9) | A lengthless `aws-chunked` streaming PUT reaches the single-PUT chunk-sizing rule | `x-amz-decoded-content-length` is optional (`sigv4.rs:579-584`) and the handler forwards only the decoded stream (`gateway-s3/src/lib.rs:1742-1764`), so the size the formula needs may not exist — the encoded body length is not the decoded size, and `DEFAULT_CHUNK_SIZE` would overflow `MAX_MAP_CHUNKS` well inside S3's 5 GiB limit. Since re-chunking mid-stream is not available, such a stream uses the **size-independent** `⌈5 GiB / MAX_MAP_CHUNKS⌉` — the value the formula yields for a maximal declared PUT — so it cannot overflow anywhere inside S3's own limit. The alternative, a mid-stream refusal, is a compatibility change for streams that work today and is recorded as rejected rather than made silently | D7 (finding 4), accepted costs |
| X103 (iteration-13 findings 1/2) | An implementer reads decision 4.2's headline sentence, or sizes `MAX_INFLIGHT_PARTS` from its stated range | Two more table-versus-prose drifts. 4.2 still opened with the blanket "written only when absent — never re-stamped", the concurrency half alone, which asserted on its own re-opens the reader-safety hole the three-arm rule below it closes. And the `W_ref/2` arithmetic for `MAX_OWNED_FLEET` silently assumed `MAX_INFLIGHT_PARTS ≤ MAX_PARTS_PER_SESSION` while the knob's range never required it — at 10,000 parts against 20,000 slots and one chunk per part the owned term is two thirds of `W_ref`, not half. The headline now states the three-arm rule; the inequality is now a clamp | D4.2, D2, D6 |
| X104 (iteration-14 finding 3) | Complete names a subset of the staged parts; a gateway drains `retire:records:{parts}` before the reaper drains the unnamed parts' `retire:bytes:` | The two obligations are drained independently, in either order, and if the records obligation named **all** the parts it could delete the unnamed parts' `part:` records first — the only thing protecting those chunks **and** the only source of their placements. The byte obligation would then have nothing to mark from, leaving durable bytes unreferenced and unevidenced: outcome (a) installed by the publication path itself. Fixed by **scope, not ordering** (a cross-obligation dependency is what this design avoids everywhere): `retire:records:{parts}` names only the **published** parts, and the unnamed parts ride `retire:bytes:{parts}`, whose payload already means *mark, then delete the naming records*. Every part record is named by exactly one obligation | D3, D4, §1 token grammar |
...
- `crates/traits/src/lib.rs` — **the one narrow-seam change this protocol requires** (ADR-0010 /
  ...
  `SCAN_CAP` — **and GC can walk `orphan:` the same way**, which iteration-12 finding 4 showed is not optional: one maximum segmented-object retirement installs ~1.78 M marks against a cap of 1,048,576, and `scan` fails whole. **The signature alone is not the contract, and the semantics are normative
...
- `crates/core/src/metadata.rs` — the new key helpers and parsers (`mpuctl:`, `slot:`, `mpu:`,
  ...
  — `seg:` (epoch-scoped, `seg:<g>:<epoch>:<i>`) and `retire:`); `PendingEntry.owner` and
...
- `crates/custodian/src/gc.rs` — **reclamation intent precedes fragment destruction**: the sweep
  CASes `orphan:<pos>` to `reclaiming` and commits **before** `delete_fragment`, deleting the key
  afterwards as today (iteration-10 finding 1) — the ordering the adoption CAS's precondition
  depends on; **`orphan_leases` and the mark sweep walk the ledger with `scan_page`, never one `scan`** (a single max-object retirement exceeds `SCAN_CAP`, iteration-12 finding 4); **`protects()` gains the pending-`retire:bytes:` class** (finding 5); plus the **three `orphan:` value variants** (legacy bare decimal, `{ orphaned_at_millis, event }`, and `{ …, reclaiming: true }` — findings 3 and 5, all three decoded, never rejected) and the **fragment-less mark sweep** (positions no `list_fragments()` reported, aged past `W_repoint + W_write + δ_clock` and observed absent *after* that deadline — findings 6, X91, X96) in `mark_orphaned`/`orphan_leases`.
- `crates/core/src/write.rs` — staged placement (committed **and** in-flight owned) selects against
  ...
  (decision 4.4); the renewal loop renews the owned `sidx:` leases **and rewrites that request's `slot:<id>:<k>`
  in the SAME batch** (iteration-10 finding 2 — the atomicity the reap fence's slot pin rests on) and
...
| **A lengthless `aws-chunked` streaming PUT cannot evaluate the sizing formula** (iteration-13 finding 9): `x-amz-decoded-content-length` is optional (`crates/gateway-s3/src/sigv4.rs:579-584`) and the handler forwards only the decoded stream (`gateway-s3/src/lib.rs:1742-1764`), so the gateway may know no object size before it starts writing — while the encoded body length is not the decoded size, and keeping `DEFAULT_CHUNK_SIZE` would overflow `MAX_MAP_CHUNKS` well inside S3's 5 GiB single-PUT limit | such a stream is written at the **size-independent** chunk size `⌈5 GiB / MAX_MAP_CHUNKS⌉` — the same value the formula yields for a *maximal* declared PUT — so no lengthless stream can overflow the flat map anywhere inside S3's own limit, and one exceeding 5 GiB is refused as S3 requires anyway. **That size must be legal for the deployment, which is a configuration precondition, not an assumption (iteration-14 finding 4):** `chunk_size_max` may be set below `⌈5 GiB / MAX_MAP_CHUNKS⌉` — nothing in the >10 GiB multipart requirement forbids it — and then the prescribed size exceeds the operator's own gateway-memory limit while *clamping* to `chunk_size_max` would let a near-5 GiB lengthless stream overflow the flat map. Neither branch is safe, so the conflict is resolved **before** any stream starts: accepting lengthless `aws-chunked` PUTs requires `chunk_size_max ≥ ⌈5 GiB / MAX_MAP_CHUNKS⌉`, checked at configuration load, and a deployment that does not meet it **refuses lengthless PUTs at the header, before streaming**, with a stated error rather than a mid-stream failure | re-chunking mid-stream is not available (chunks are written as they arrive), so the choice is a fixed size that is always safe or a mid-stream refusal; refusing would be a **compatibility change** for streams that succeed today, and this proposal is not the place to make one silently. The cost is that small lengthless PUTs use larger chunks than they need — read amplification, not correctness | **FU-5**/operator `chunk_size` tuning, and telemetry on the lengthless-stream share |
| **Single-PUT chunk size scales with object size** (segmentation is multipart-only, finding 4): a large single `PutObject` uses `chunk_size_effective = max(DEFAULT_CHUNK_SIZE, ⌈Content-Length/MAX_MAP_CHUNKS⌉)` to stay flat; a `PutObject` that cannot fit `MAX_MAP_CHUNKS` at `chunk_size_max` is refused; a **lengthless** `aws-chunked` stream, which cannot evaluate the formula at all, uses the size-independent `⌈5 GiB / MAX_MAP_CHUNKS⌉` (iteration-13 finding 9) | S3's 5 GiB single-PUT max fits once `chunk_size ≥ 5 GiB/MAX_MAP_CHUNKS ≈ **13.4–31 MiB**`; the gateway-memory cost is `chunk_size × max_concurrent_encodes` (bounded, and only for large single PUTs) | segmentation needs a session/epoch a single PUT has not got; chunk-size selection is the simple, sessionless way to reach S3's single-PUT ceiling, and refusal past it is a safe guard, never a silent unanchored segmentation (X50) | **FU-5**/operator `chunk_size` tuning (same trade as the max-part-size row) |
...
- **Tier-0 DST ([ADR-0009][a9]) is the correctness authority** for every interleaving named in
  ...
  **the `sidx:`-renews-before-the-slot split-batch race (X74), the legacy-`orphan:`-value contact
  path (X75), the ambiguous Create recovered under the same upload id (X78), the over-age
  `Completing` ceiling (X79), and the tombstone answering a non-identical Complete (X81)**,
  **the intent-without-slot-rewrite fence race (X83), the self-renewal-versus-own-commit race
  (X84), the refused-part compensation (X85), the crash between GC's `reclaiming` CAS and its
  `delete_fragment` (X86), the fragment-less orphan-mark sweep (X87), and the paused-worker
  pre-mark reclamation (X88)**, **the sweep-versus-pre-mark composition (X91), the
  stale-mark-unprotected-before-the-drain delete/read case (X92), and the own-renewal commit
  ...
  per-backend timing case** — a batch at the byte budget must commit inside the 5-second half of
  the envelope on every backend, which is a conformance property, not a DST one. The `scan_page` ordering and
...
- **The classification sweep** — one test helper this protocol earns: given a store and a fleet,
  assert every on-disk fragment is in **at least one** safe class — committed-referenced,
  staged-with-a-session, or evidenced-for-reclamation — and separately that it is in no
  *genuinely incompatible* combination (evidenced-for-reclamation with its grace already elapsed
  **while** still committed-referenced is the one that matters, since that is the pair GC would act
  on wrongly). **"Exactly one" would be wrong, and would fail on correct executions (iteration-12
  finding 2):** this protocol *deliberately* overlaps protection across both handoffs — after the
  root flip and before `retire:records:{parts}` drains, every published chunk is simultaneously
  committed-referenced **and** named by a staged `part:` record, and a retirement's marks precede
  its record deletions by construction, so a chunk is briefly staged **and** evidenced. That
  overlap is exactly what closes the gaps the source-before-destination read orders exist to
  prevent (X67, decision 2), so a helper demanding disjointness would pressure an implementer to
  remove the very property that makes the handoffs safe. Invariant (2) is a **no-gaps** claim, not
  a partition. Run it after every DST scenario.
...
- **On-disk / record format.** The one *non*-additive change is the `orphan:` **value** (above: bare decimal → `{ orphaned_at_millis, event }`, plus GC's `reclaiming: true` variant — all three decoded on read, none rejected). Otherwise additive prefixes (`mpuctl:`, `mpu:`, `part:`, `sidx:`, `seg:`,
...
- **Version skew during rollout — a hard ordering requirement.** A custodian build **without**
  ...
  absent).
- **The `orphan:` value gains a field (iteration-10 finding 3).** Values are bare decimal
  `orphaned_at_millis` today (`gc.rs:110-122`, parsed at `:315-330`) and become
  `{ orphaned_at_millis, event }`, and GC's in-progress reclamation adds a `reclaiming: true`
  variant (decision 4.2). Decoding accepts all three and rejects none — a decode error here aborts
  the whole GC pass. A bare decimal is a legacy mark
  with no identity, which the three-arm rule treats as a *different* identity and replaces on first
  contact, so there is no migration pass and no ledger rewrite by a reader. A downgraded custodian
  fails to parse a structured value and drops it from `orphan_leases`, leaving the fragment
  unreclaimable rather than wrongly reclaimed — safe, and covered by the custodian-before-gateway
  ordering above. Round-trip coverage for both forms is owed with the change (decision 4.2).
- **Metadata restore.** The restore tool MUST fence every restored `Open`/`Completing` session to
  ...
  `retire:bytes:{session, parts}` **and** `retire:records:{seg:<g>:<E>}` so that any segments that
