...
`reconstruction_repaired` is incremented **up front**, at the assessment frame, because the rebuild runs a heavy erasure-decode plus a version-conditional commit and a metric emitted *after* that section is unreliable on the `tracing`→OTel bridge under load. The two **terminal non-success outcomes** are each offset on their own counter, giving the **success-netting approximation**:

> **successful repairs ≈ `reconstruction_repaired` − `reconstruction_conflict` − `reconstruction_aborted`**

Read this as a **convergence indicator, not an exact conservation law.** It is exact only when every dispatched repair reaches a terminal outcome — `Committed`, or one of the two offset outcomes `Conflict` / `Aborted`. A repair that instead **errors out** of the pass — no spare failure domain to place the rebuild (`select_distinct_domains_excluding`), or an I/O failure on the fragment write or the version-conditional commit — propagates *after* its up-front `reconstruction_repaired` increment and emits **neither** offset, so over a window the formula **over-counts** committed repairs by the number of such errored attempts (the chunk stays queued and is re-assessed next pass). `reconstruction_aborted` (#197 / #238) closed the "selector chose a server outside the fleet view" leak; the remaining errored paths are a **known accounting imprecision that is not yet offset** — treat a persistent gap between the netted figure and a falling `under-replicated count` as a prompt to inspect, not as a tally of lost data.

These three counters **complement, not replace,** the five higher-level durability metrics of §1: a non-zero `reconstruction_conflict` / `reconstruction_aborted` rate while `under-replicated count` is not falling is the signal that the loop is churning rather than converging. They are emitted from the custodians' first commit; the source of truth for their exact emission is `crates/custodian/src/reconstruction.rs` (`emit_repaired` / `emit_conflict` / `emit_aborted`), and they sit alongside the proposal 0005 §"durability metrics" set.
