...
The multipart commit protocol (proposal 0016) adds its own first-class records under disjoint key prefixes: the `mpuctl` admission ledger singleton (a `Budget` profile and the session count it derives `MAX_SESSIONS` from), and per-upload `mpu:<id>` (a session's target, lifecycle state and epoch), `slot:<id>:<k>` (an in-flight part reservation), `part:<id>:<n>` (a committed part's chunk list, length and digest) and `psum:<id>:<n>` (that part's summary) — every one of these validated structurally at decode (ADR-0045) rather than trusted on successful JSON parsing alone, so a stored record whose fields disagree with each other (a session whose publish target names another object or another fence epoch, a part whose length is not its chunks' summed length, a slot born already lapsed) is a typed error rather than a live value. A session transition is a compare-and-set on the record's exact bytes, so the same omitted-when-absent rule the inode's optional trio follows governs the session's optional `content_type`: absent is spelled by omitting the field, and neither decode nor encode may rewrite the other spelling. The same rule reaches one level in — a part record's chunk values are read through a closed wire shape of their own, so an unknown or omitted chunk field is a decode error rather than a silent rewrite of the record under its own compare-and-set.
