...
The record **types** of the multipart commit protocol (proposal 0016, still a draft) are landed ahead of their writers, under disjoint key prefixes: the `mpuctl` admission ledger singleton (a `Budget` profile and the session count it derives `MAX_SESSIONS` from), and per-upload `mpu:<id>` (a session's target, lifecycle state and epoch), `slot:<id>:<k>` (an in-flight part reservation), `part:<id>:<n>` (a committed part's chunk list, length and digest) and `psum:<id>:<n>` (that part's summary), and the reclamation pair `retire:bytes:<token>` (bytes to orphan-mark, then the records that named them) and `retire:records:<token>` (records to delete whose bytes something else still protects), whose value names what one obligation owes — a session's staged residue, a range-encoded set of its part numbers, one part attempt's chunk list, a superseded generation (by the chunk list a flat root carried, or by the segment group a segmented one named — the two shapes a published map has, never both) or a rolled-back completion attempt's segment records, in the combinations the protocol installs as single obligations — every one validated structurally at decode (ADR-0045) rather than trusted on successful JSON parsing alone, so a stored record whose fields disagree with each other (a session whose publish target names another object or another fence epoch, a part whose length is not its chunks' summed length, a slot born already lapsed) is a typed error rather than a live value. A retirement obligation's identity lives partly in its `<token>` — which session at which fence epoch, which part attempt of it, which superseded generation — so its value is decoded **against its own key**, and a payload whose mode, token scope, generation identity or segment epoch disagrees with the token naming it is likewise a typed error rather than an obligation that would reclaim one attempt's bytes while clearing another's. The owned staging entry `sidx:<id>:<n>:<chunk>` is the lease record a streaming write stores under `pending:<chunk>`, plus the owning upload id (`owner`) and the chunk's planned placement (`staged` — an erasure scheme the coder must support, and a D server per planned fragment, a count that decode leaves unchecked as for a committed chunk). Both are present on an owned entry and absent on an ordinary lease, and each is omitted when absent, so a `pending:` record written before them decodes and re-encodes byte-identically — load-bearing because a renewal puts a freshly encoded entry over the bytes it read. A value whose fields disagree with each other or with its key (one field alone, an owned entry under `pending:`, an ordinary lease under `sidx:`, an owner other than the key's) is refused by every reader and both `pending:` writers, and an expiry sweep skips it without reclaiming its chunk; GC's expired-lease input, off unless an operator arms it, also names it on the audit seam, and the post-restore reconciliation holds its chunk unmarked and reports the entry as needing a human, as it does an unreadable chunk map. An ownership field spelled `null` is refused too: the encoder never emits that spelling, so a lease that carried it would be rewritten by its next renewal. Nothing writes or consumes these records in production yet — the protocol itself (fenced state transitions, staged publication, retirement) arrives with the store round trip (#656–#659) and is specified in the proposal, not here. Because every transition it specifies is a compare-and-set on the record's exact bytes, the decoders enforce serialization identity now, before any writer exists to depend on it: the accepted set is exactly the encoder's image, byte for byte — an absent `content_type` is spelled only by omitting the field (the inode trio's omitted-when-absent rule), a noncanonical spelling of an accepted value (fields reordered, whitespace inserted) is a decode error rather than bytes a CAS could never match, and a part record's chunk values are read through a closed wire shape of their own, so an unknown or omitted chunk field is a decode error rather than a silent rewrite.

The protocol's client-visible answers, and the two digests a completed upload is identified by, are landed the same way: as pure functions that no production path calls yet. A multipart object's ETag is composed on ADR-0047's SHA-256 basis, never MD5: the lowercase-hex SHA-256 of the named parts' raw 32-byte digests concatenated in part-number order, then `-` and the number of parts named (`<hex>-<N>`, with `N` from 1 to 999,999, the most parts the `part:` key grammar can address). A `Completed` session record's `etag` holds that ETag whole, suffix included, and decode refuses any other spelling (a bare digest, a count out of range, a noncanonical number), because by the time the session's tombstone answers a retry, the part records the ETag was composed from may already be retired. Beside it, `complete_fingerprint` is a SHA-256 over each named part's number (4 bytes, big-endian) followed by its digest, so the tombstone answers an identical retry with the recorded ETag and a different part list under the consumed upload id with `NoSuchUpload`. Both digests are computed only over a non-empty list in strictly ascending part-number order; any other list is refused, never sorted. Proposal 0016's decision-3 table answers its five verbs (`UploadPart`, `CompleteMultipartUpload`, `AbortMultipartUpload`, `ListParts`, `ListMultipartUploads`) in each session state and for an absent record, in a typed outcome vocabulary that names no HTTP status; the S3 status and XML mapping arrives with #508.
...
The store's **read surface is two operations**, and the split is a durability requirement rather than an optimization. `scan(prefix)` returns a whole namespace as one consistent cut and **fails loud** past its per-listing cap instead of truncating — a silently short `inode:` listing would shrink GC's never-reclaim safety set, which is data loss (#262, ADR-0011). `scan_page(prefix, after, limit)` walks a namespace **larger than that cap**, in cursor-keyed pages ordered by raw byte-lexicographic key, with an exclusive cursor and a continuation that may not skip a key present throughout the walk (#634). The second exists because two multipart-era populations outgrow the cap — the deliberately unbounded retirement drain, and GC's orphan ledger, where one maximum segmented-object retirement installs ~1.78 M marks against a cap of 1,048,576 — and a namespace that cannot be enumerated cannot be reclaimed. Both contracts are normative for *every* backend and asserted on all of them by the shared conformance suite (`crates/metadata-conformance`), which is what keeps redb, FoundationDB and TiKV from disagreeing about how a listing behaves.
