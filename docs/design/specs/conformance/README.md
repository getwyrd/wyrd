...
other mechanically: the `chunk-format` crate is run against these vectors in CI
...
- Adding or changing a vector is part of the strict spec change process (a format
  version bump). Vectors are append-only within a format version.

These vectors are also how a future non-Rust client (ADR-0002, architecture
section 8.6) proves it reads and writes data compatibly, rather than relying on
reverse-engineering the reference code.
