...
fn assert_chunk_map_is_whole(settled: &InodeRecord, prior: &InodeRecord, plans: &[Vec<ChunkRef>]) {
    let whole =
        std::iter::once(prior.chunk_map.as_flat().unwrap()).chain(plans.iter().map(Vec::as_slice));
    let settled_map = settled.chunk_map.as_flat().unwrap();
    assert!(
        whole.into_iter().any(|map| map == settled_map),
...
async fn deferred_1031_settles_against_current_truth(meta: Arc<SimFdbMetadataStore>) -> bool {
    ...
    if after_b.chunk_map.as_flat().unwrap() != b.chunk_refs().as_slice() {
        return false;
    }
